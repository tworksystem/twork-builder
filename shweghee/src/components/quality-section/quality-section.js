...
function renderStep( item ) {
	return `
    <div class="quality-section__step" data-item-id="${ escapeHtml(
		item.id
	) }">
      <span class="quality-section__step-num">${ escapeHtml(
			item.number
		) }</span>
...
function renderStandard( item ) {
	return `
    <div class="quality-section__standard" data-item-id="${ escapeHtml(
		item.id
	) }">
      <span class="quality-section__standard-icon" aria-hidden="true">✓</span>
      <div>
        <h3 class="quality-section__standard-title">${ escapeHtml(
			item.title
		) }</h3>
        <p class="quality-section__standard-text">${ escapeHtml(
			item.text
		) }</p>
      </div>
    </div>
...
function buildTemplate( data ) {
	...
	const image = imageSrc
		? `<img class="quality-section__image" src="${ imageSrc }" alt="${ escapeHtml(
				q.imageAlt || ''
      ...
      <div class="quality-section__intro" id="standards">
        <div class="quality-section__intro-text">
          ...
          ${ ( q.paragraphs || [] )
				.map(
					( p ) =>
						`<p class="quality-section__para">${ escapeHtml(
							p
						) }</p>`
				)
				.join( '' ) }
...
export function initQualitySection( rootEl, data = {} ) {
	...
	rootEl
		.querySelectorAll( '.quality-section__image' )
