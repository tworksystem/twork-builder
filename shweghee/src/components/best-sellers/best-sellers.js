...
function renderProductSlide( products, index ) {
	...
	const cards = ( products || [] )
		...
		.join( '' );
	return `
    <div class="product-carousel__slide${ active }" data-carousel-slide aria-hidden="${
		index === 0 ? 'false' : 'true'
	}">
		...
		.map(
			( _, i ) =>
				`<button type="button" class="carousel-dots__dot${
					i === 0 ? ' is-active' : ''
				}" data-action="carousel-go" data-slide-index="${ i }" aria-label="Slide ${
					i + 1
				}"></button>`
		)
		.join( '' );
    ...
    <div class="shop-panel best-sellers__panel">
      <header class="shop-panel__head">
        <div class="shop-panel__title-wrap">
          ...
          <h2 class="shop-panel__title">${ escapeHtml(
				section.title || ''
			) }</h2>
        </div>
      ...
      <div class="best-sellers__stage" role="region" aria-label="${ escapeHtml(
			section.title || ''
		) }">
        <div class="best-sellers__track">${ slides
			.map( renderProductSlide )
			.join( '' ) }</div>
      </div>
