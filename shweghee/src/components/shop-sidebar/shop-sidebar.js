...
function renderCategoryItem( cat ) {
	const children = ( cat.children || [] )
		.map(
			( child ) =>
				`<a class="shop-sidebar__child-link" href="${ safeUrl(
					child.href
				) }" data-item-id="${ escapeHtml( child.id ) }">${ escapeHtml(
					child.label
				) }</a>`
		)
		.join( '' );
	...
	return `
    <li class="shop-sidebar__cat-item" data-accordion-item data-open="${
		cat.open ? 'true' : 'false'
	}">
...
function renderRecommendationSlide( products, index ) {
	...
	const cards = ( products || [] )
		...
		.join( '' );
	return `
    <div class="product-carousel__slide shop-sidebar__rec-slide${ active }" data-carousel-slide aria-hidden="${
		index === 0 ? 'false' : 'true'
	}">
...
function buildTemplate( data ) {
		...
		.map(
			( _, i ) =>
				`<button type="button" class="carousel-dots__dot${
					i === 0 ? ' is-active' : ''
				}" data-action="carousel-go" data-slide-index="${ i }" aria-label="Recommendations page ${
					i + 1
				}"></button>`
		)
		.join( '' );
    ...
    <aside class="shop-sidebar__inner">
      <div class="shop-sidebar__widget shop-panel">
        <h2 class="shop-sidebar__widget-title">${ escapeHtml(
			s.categoriesTitle || 'PRODUCT CATEGORIES'
		) }</h2>
      ...
      <div class="shop-sidebar__widget shop-panel">
        <h2 class="shop-sidebar__widget-title">${ escapeHtml(
			s.recommendationsTitle || 'RECOMMENDATIONS'
		) }</h2>
