...
const INFO_ICONS = {
	...
	clock: '<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
};
...
function renderInfoCard( card ) {
	...
	return `
    <div class="contact-form__info-card" data-item-id="${ escapeHtml(
		card.id
	) }">
...
function buildTemplate( data ) {
    ...
    <div class="contact-form__inner l-section">
      ...
      <div class="contact-form__panel">
        <h2 class="contact-form__heading">${ escapeHtml(
        ...
        <form class="contact-form__form" data-action="contact-submit" novalidate>
          <input type="text" name="${ escapeHtml(
				c.honeypotName || 'company'
			) }" class="contact-form__hp u-hidden" tabindex="-1" autocomplete="off" aria-hidden="true" />
          ...
          <button class="btn btn--primary contact-form__submit" type="submit">${ escapeHtml(
				c.submitLabel || 'SEND MESSAGE'
			) }</button>
...
export function initContactForm( rootEl, data = {} ) {
	...
	on( rootEl, 'submit', "[data-action='contact-submit']", ( event ) => {
		...
		feedback.textContent =
			'Demo: သင့်မက်ဆေ့ချ် မှတ်တမ်းတင်မည်မဟုတ်ပါ (backend မချိတ်ရသေးပါ)။ ဖုန်းဖြင့် ဆက်သွယ်ပါ — 095-2-55122';
		form.reset();
