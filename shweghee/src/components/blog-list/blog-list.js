...
function renderPost( post ) {
    ...
    <article class="blog-list__card" data-item-id="${ escapeHtml( post.id ) }">
      ...
      <p class="blog-list__meta">
        ...
        on <time datetime="${ escapeHtml( post.dateIso || '' ) }">${ escapeHtml(
			post.date
		) }</time>
      </p>
      <h2 class="blog-list__title"><a href="${ safeUrl(
			post.href
		) }">${ escapeHtml( post.title ) }</a></h2>
      <p class="blog-list__excerpt">${ escapeHtml( post.excerpt ) }</p>
      <a class="blog-list__read-more" href="${ safeUrl(
			post.href
		) }">Read more →</a>
    </article>
  `;
...
function buildTemplate( data ) {
    ...
    <div class="blog-list__inner l-section">
      ...
      ${
			posts.length === 0
				? `<p class="blog-list__empty">No posts available.</p>`
				: ''
		}
    </div>
  `;
