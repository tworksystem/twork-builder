...
function renderCard( item ) {
	...
	const media = imageSrc
		? `<img src="${ imageSrc }" alt="${ escapeHtml(
				item.imageAlt || ''
		  ) }" width="380" height="280" loading="lazy" decoding="async" />`
		: '';
    ...
    <article class="services-carousel__card" data-item-id="${ item.id }">
      ...
      <div class="services-carousel__box">
        ...
        <a class="services-carousel__link" href="${ safeUrl( item.href ) }">${
		...
		.map(
			( _, i ) =>
				`<button type="button" class="carousel-dots__dot${
					i === 0 ? ' is-active' : ''
				}" data-action="carousel-go" data-slide-index="${ i }" aria-label="Service ${
					i + 1
				}"></button>`
		)
		.join( '' );
