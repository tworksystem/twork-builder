...
function renderPrice( product ) {
	if ( product.salePrice != null ) {
		return `
      <span class="product-tabs__price product-tabs__price--sale" data-field="salePrice">${ formatCurrency(
			product.salePrice
		) }</span>
      <span class="product-tabs__price product-tabs__price--old" data-field="price">${ formatCurrency(
			product.price
		) }</span>
    `;
	}
	return `<span class="product-tabs__price" data-field="price">${ formatCurrency(
		product.price
	) }</span>`;
}
...
function renderProductCard( product ) {
    ...
    <article class="product-tabs__card" data-item-id="${ product.id }">
      ...
      <button class="product-tabs__add btn" type="button" data-action="add-cart" data-product-id="${
			product.id
		}">Add to cart</button>
    </article>
  `;
...
function renderPanel( tabId, products, isActive ) {
    ...
    <div
      class="product-tabs__panel${
			isActive ? ' product-tabs__panel--active' : ''
		}"
      role="tabpanel"
      data-panel-id="${ tabId }"
      ${ isActive ? '' : 'hidden' }
...
function buildTemplate( data ) {
	...
	const tabButtons = tabs
      ...
      <button
        class="product-tabs__tab${
			tab.id === firstId ? ' product-tabs__tab--active' : ''
		}"
        type="button"
        role="tab"
	...
	const panels = tabs
		.map( ( tab ) =>
			renderPanel( tab.id, productsByTab[ tab.id ], tab.id === firstId )
		)
		.join( '' );
