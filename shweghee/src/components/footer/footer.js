...
const INFO_ICONS = {
	...
	clock: '<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
};
...
function renderSocialLink( item ) {
	...
	return `<a class="footer__social-link" href="${ safeUrl(
		href
	) }" aria-label="${ label }" rel="noopener noreferrer" target="_blank">${ initial }</a>`;
}
...
function renderColumn( column ) {
	const links = ( column.links || [] )
		.map(
			( link ) =>
				`<a class="footer__link" href="${ safeUrl(
					link.href
...
function buildTemplate( data ) {
	...
	const pastureImg = pastureSrc
		? `<img src="${ pastureSrc }" alt="${ escapeHtml(
				f.pastureImageAlt || ''
		  ) }" width="1600" height="400" loading="lazy" decoding="async" />`
		: '';
    ...
    <div class="footer__main l-section">
      <div class="footer__brand-col">
        <a class="footer__brand" href="${ safeUrl( '/pages/home.html' ) }">
          <span>${ escapeHtml(
				f.brandPrefix || 'Farm'
			) }</span><span class="footer__brand-suffix">${ escapeHtml(
				f.brandSuffix || 'art.'
			) }</span>
        </a>
