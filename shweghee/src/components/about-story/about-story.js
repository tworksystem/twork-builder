...
function renderMilestone( item ) {
	return `
    <div class="about-story__milestone" data-item-id="${ escapeHtml(
		item.id
	) }">
      <span class="about-story__year">${ escapeHtml( item.year ) }</span>
      <h3 class="about-story__milestone-title">${ escapeHtml(
			item.title
		) }</h3>
...
function buildTemplate( data ) {
	...
	const image = imageSrc
		? `<img class="about-story__image" src="${ imageSrc }" alt="${ escapeHtml(
				s.imageAlt || ''
      ...
      <div class="about-story__intro" id="story">
        <div class="about-story__intro-text">
          ...
          ${ ( s.paragraphs || [] )
				.map(
					( p ) =>
						`<p class="about-story__para">${ escapeHtml( p ) }</p>`
				)
				.join( '' ) }
          ${
				s.ctaLabel
