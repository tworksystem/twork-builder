	...
	return links
		.map(
			( link ) =>
				`<a href="${ safeUrl(
					link.href
				) }" data-item-id="${ escapeHtml( link.id ) }">${ escapeHtml(
					link.label
				) }</a>`
		)
		.join( '' );
	...
	return items
		.map( ( item ) => {
			const chevron = item.hasDropdown
				? '<span class="shop-header__chevron" aria-hidden="true"></span>'
				: '';
			return `<a class="shop-header__nav-link" href="${ safeUrl(
				item.href
			) }" data-item-id="${ escapeHtml( item.id ) }">${ escapeHtml(
				item.label
			) }${ chevron }</a>`;
		} )
		.join( '' );
...
function buildTemplate( data ) {
	...
	const logoSrc = safeImageSrc(
		h.logoSrc || '../assets/brand/shwe-myanmar-logo.png'
	);
	const logo = logoSrc
		? `<img
            ...
            decoding="async"
          />`
		: `<span class="shop-header__brand-text">${ escapeHtml(
				h.brandName || 'Shwe Myanmar'
		  ) }</span>`;
    ...
    <div class="shop-header__utility">
      <div class="shop-header__utility-inner l-section shop-header__container">
        ...
        <nav class="shop-header__utility-nav" aria-label="Utility">
          ...
          <button type="button" class="shop-header__lang" data-field="language">${ escapeHtml(
				h.language || 'English'
			) } ▾</button>
          <button type="button" class="shop-header__currency" data-field="currency">${ escapeHtml(
				h.currency || 'MMK'
			) } ▾</button>
        </nav>
      </div>
    ...
    <div class="shop-header__main">
      <div class="shop-header__main-inner l-section shop-header__container">
        <a class="shop-header__brand" href="${ safeUrl(
			'/pages/home.html'
		) }" aria-label="${ escapeHtml( h.brandName || 'Shwe Myanmar' ) } home">
          ${ logo }
        </a>
        <form class="shop-header__search" role="search" action="#" method="get">
          ...
          <input id="shop-search" class="shop-header__search-input" type="search" name="q" placeholder="${ escapeHtml(
				h.searchPlaceholder || 'Search...'
			) }" maxlength="120" />
        ...
        <div class="shop-header__icons">
          ...
          <a class="shop-header__cart" href="#" aria-label="Shopping cart">
            ...
            <span class="shop-header__cart-total" data-field="cartTotal">${ escapeHtml(
				h.cartTotal || '0 MMK'
			) }</span>
          </a>
        </div>
    ...
    <div class="shop-header__nav-bar">
      <div class="shop-header__nav-inner l-section shop-header__container">
        ...
        <nav id="shop-header-nav" class="shop-header__nav" aria-label="Main">${ renderMenuItems(
			h.menuItems
		) }</nav>
        <a class="shop-header__recent" href="#">${ escapeHtml(
			h.recentLabel || 'Recently Viewed'
		) }</a>
      </div>
    </div>
  `;
...
export function initShopHeader( rootEl, data = {} ) {
	...
	rootEl
		.querySelector( '.shop-header__search' )
