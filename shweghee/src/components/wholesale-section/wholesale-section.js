...
function renderBenefit( item ) {
	return `
    <div class="wholesale-section__benefit" data-item-id="${ escapeHtml(
		item.id
	) }">
      <span class="wholesale-section__benefit-icon" aria-hidden="true">✓</span>
      <div>
        <h3 class="wholesale-section__benefit-title">${ escapeHtml(
			item.title
		) }</h3>
        <p class="wholesale-section__benefit-text">${ escapeHtml(
			item.text
		) }</p>
      </div>
    </div>
...
function buildTemplate( data ) {
	...
	const image = imageSrc
		? `<img class="wholesale-section__image" src="${ imageSrc }" alt="${ escapeHtml(
				w.imageAlt || ''
		  ) }" width="480" height="360" loading="lazy" decoding="async" />`
		: '';

	const products = ( w.featuredProducts || [] )
    ...
    <div class="wholesale-section__inner l-section">
      <div class="wholesale-section__hero-grid">
        <div>
          ...
          <p class="wholesale-section__intro">${ escapeHtml(
				w.intro || ''
			) }</p>
      ...
      <div class="wholesale-section__products">
        <h2 class="wholesale-section__products-title">${ escapeHtml(
			w.productsTitle || 'Bulk Products'
		) }</h2>
        <div class="wholesale-section__products-grid">${ products }</div>
      </div>

      <div class="wholesale-section__form-panel">
        <h2 class="wholesale-section__form-title">${ escapeHtml(
			w.formTitle || 'Wholesale Inquiry'
		) }</h2>
        <p class="wholesale-section__form-desc">${ escapeHtml(
			w.formDesc || ''
		) }</p>
        <form class="wholesale-section__form" data-action="wholesale-submit" novalidate>
          <input type="text" name="${ escapeHtml(
				w.honeypotName || 'url'
			) }" class="wholesale-section__hp u-hidden" tabindex="-1" autocomplete="off" aria-hidden="true" />
...
export function initWholesaleSection( rootEl, data = {} ) {
	...
	rootEl
		.querySelectorAll( '.wholesale-section__image' )
		.forEach( imageOnError );
	...
	on( rootEl, 'submit', "[data-action='wholesale-submit']", ( event ) => {
		...
		feedback.textContent =
			'Demo: စုံစမ်းမှု မှတ်တမ်းတင်မည်မဟုတ်ပါ။ ဖုန်းဖြင့် ဆက်သွယ်ပါ — 095-2-55122';
		form.reset();
