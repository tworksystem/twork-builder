...
function renderRegion( item ) {
    ...
    <div class="where-to-buy__region" data-item-id="${ escapeHtml( item.id ) }">
      ...
      <p class="where-to-buy__region-desc">${ escapeHtml(
			item.description
		) }</p>
      ${
			outlets
				? `<ul class="where-to-buy__region-outlets">${ outlets }</ul>`
				: ''
		}
    </div>
  `;
    ...
    <div class="where-to-buy__inner l-section">
      <div class="where-to-buy__intro">
        ...
        ${ ( w.paragraphs || [] )
			.map(
				( p ) =>
					`<p class="where-to-buy__para">${ escapeHtml( p ) }</p>`
			)
			.join( '' ) }
      </div>
      ...
      <div class="where-to-buy__cta">
        <h2 class="where-to-buy__cta-title">${ escapeHtml(
			w.ctaTitle || ''
		) }</h2>
        ...
        <div class="where-to-buy__cta-actions">
          ${
				w.ctaPhone
					? `<a class="btn btn--primary" href="tel:${ w.ctaPhone.replace(
							/\D/g,
							''
					  ) }">${ escapeHtml( w.ctaPhoneLabel || w.ctaPhone ) }</a>`
					: ''
			}
          ${
				w.ctaHref
					? `<a class="btn btn--outline" href="${ safeUrl(
							w.ctaHref
					  ) }">${ escapeHtml( w.ctaLabel || 'Contact Us' ) }</a>`
					: ''
			}
        </div>
      </div>
