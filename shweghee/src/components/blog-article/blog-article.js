...
function renderRelated( post ) {
	return `
    <a class="blog-article__related-card" href="${ safeUrl(
		post.href
	) }" data-item-id="${ escapeHtml( post.id ) }">
...
function buildTemplate( article, related = [] ) {
	...
	const image = imageSrc
		? `<img class="blog-article__image" src="${ imageSrc }" alt="${ escapeHtml(
				article.imageAlt || ''
		  ) }" width="800" height="450" loading="eager" decoding="async" />`
		: '';
    ...
    <article class="blog-article__inner l-section">
      <header class="blog-article__header">
        <p class="blog-article__meta">
          ...
          on <time datetime="${ escapeHtml(
				article.dateIso || ''
			) }">${ escapeHtml( article.date ) }</time>
        </p>
      ...
      ${
        ...
        <aside class="blog-article__related">
          ...
          <div class="blog-article__related-grid">${ related
				.map( renderRelated )
				.join( '' ) }</div>
        </aside>
      `
