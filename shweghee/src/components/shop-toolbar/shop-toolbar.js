...
function buildTemplate( data ) {
    ...
    <div class="shop-toolbar__page-info" data-field="pageInfo">
      Page <span class="shop-toolbar__page-box"><input type="text" inputmode="numeric" value="${
			t.currentPage || 1
		}" aria-label="Current page" data-action="page-input" /></span> of ${
			t.totalPages || 1
		}
    </div>
...
export function getToolbarState( toolbarEl ) {
	...
	const perPageSelect = toolbarEl?.querySelector(
		"[data-action='per-page']"
	);
	const sortSelect = toolbarEl?.querySelector( "[data-action='sort']" );
	const layoutBtn = toolbarEl?.querySelector(
		'.shop-toolbar__layout.is-active'
	);

	let page =
		Number.parseInt( pageInput?.value, 10 ) || config.currentPage || 1;
	...
	return {
		...
		totalPages,
		perPage:
			Number.parseInt( perPageSelect?.value, 10 ) ||
...
export function bindToolbarEvents( toolbarEl, onChange ) {
	...
	toolbarEl.addEventListener( 'change', ( e ) => {
		if (
			e.target.matches( "[data-action='sort'], [data-action='per-page']" )
		) {
			emit();
		}
	} );
	...
	toolbarEl.addEventListener( 'keydown', ( e ) => {
		if (
			e.target.matches( "[data-action='page-input']" ) &&
			e.key === 'Enter'
		) {
			e.target.blur();
		}
