...
function renderMenuItems( items ) {
	return items
		.map( ( item ) => {
			const dropdown = item.hasDropdown
				? ' <span class="icon-chevron" aria-hidden="true"></span>'
				: '';
			return `<a class="header__nav-link" href="${ safeUrl(
				item.href
			) }" data-item-id="${ item.id }">${ item.label }${ dropdown }</a>`;
		} )
		.join( '' );
}
...
function buildTemplate( data ) {
	...
	const logo = logoSrc
		? `<img
            ...
            decoding="async"
          />`
		: `<span class="header__brand-text">${ escapeHtml(
    ...
    <div class="header__main">
      <div class="header__inner l-section">
        <a class="header__brand" href="${ safeUrl(
			'/pages/home.html'
        ...
        <div class="header__actions">
          ...
          <div class="header__hotline">
            ...
            <div>
              ...
              <a class="header__hotline-phone" href="tel:${ (
					h.phone || ''
				).replace( /\D/g, '' ) }">${ h.phone || '' }</a>
            </div>
          </div>
...
export function initHeader( rootEl, data = {} ) {
	...
	document.addEventListener( 'keydown', ( e ) => {
		if (
			e.key === 'Escape' &&
			rootEl.classList.contains( 'header--open' )
		) {
