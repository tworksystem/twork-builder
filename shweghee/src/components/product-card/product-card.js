...
function renderBadge( badge ) {
	...
	return `<span class="product-card__badge ${ type }">${ escapeHtml(
		badge.text
	) }</span>`;
}
...
function renderPrice( product ) {
	if ( product.priceRange ) {
		...
		return `<p class="product-card__price product-card__price--range">${ formatCurrency(
			min
	...
	if ( product.salePrice != null && product.price != null ) {
      ...
      <p class="product-card__price">
        <span class="product-card__price--sale">${ formatCurrency(
			product.salePrice
		) }</span>
        <span class="product-card__price--was">${ formatCurrency(
			product.price
		) }</span>
      </p>
    `;
...
export function renderProductCard( product, options = {} ) {
	...
	if ( variant === 'compact' ) {
		return `
      <article class="product-card product-card--compact" data-item-id="${ escapeHtml(
			product.id
		) }">
        <a class="product-card__thumb-link" href="${ safeUrl( product.href ) }">
          <img class="product-card__thumb" src="${ escapeHtml(
				product.image || ''
			) }" alt="${ escapeHtml(
				product.imageAlt || product.title
			) }" width="80" height="80" loading="${ loading }" />
        </a>
        <div class="product-card__body">
          <h3 class="product-card__title"><a href="${ safeUrl(
				product.href
			) }">${ escapeHtml( product.title ) }</a></h3>
	...
	return `
    <article class="product-card product-card--grid" data-item-id="${ escapeHtml(
		product.id
	) }">
      <div class="product-card__media">
        ${ renderBadge( product.badge ) }
        <a href="${ safeUrl( product.href ) }" class="product-card__image-link">
          <img class="product-card__image" src="${ escapeHtml(
				product.image || ''
			) }" alt="${ escapeHtml(
				product.imageAlt || product.title
			) }" width="280" height="280" loading="${ loading }" />
        </a>
      </div>
      <div class="product-card__body">
        <h3 class="product-card__title"><a href="${ safeUrl(
