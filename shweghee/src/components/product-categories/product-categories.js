...
function renderCard( item ) {
	...
	const image = imageSrc
		? `<img class="product-categories__img" src="${ imageSrc }" alt="${ escapeHtml(
				item.imageAlt || ''
		  ) }" width="140" height="140" loading="lazy" decoding="async" />`
		: '';

	return `
    <a class="product-categories__card" href="${ safeUrl(
		item.href
...
function buildTemplate( data ) {
    ...
    <div class="product-categories__inner l-section">
      ...
      <div class="product-categories__grid" data-list="items">
        ...
        <a class="product-categories__card product-categories__card--cta" href="${ safeUrl(
			c.ctaHref
		) }">
