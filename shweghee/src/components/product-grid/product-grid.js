...
import {
	getToolbarState,
	bindToolbarEvents,
} from '../shop-toolbar/shop-toolbar.js';
...
function renderGrid( products, layout ) {
	...
	return `<div class="${ layoutClass(
		layout
	) }" data-list="products" id="products">${ cards }</div>`;
}
...
function renderPagination( page, totalPages ) {
    ...
    <nav class="product-grid__pagination" aria-label="Product pagination">
      ...
      <button type="button" class="product-grid__page-next" data-action="grid-page-next" aria-label="Next page"${
...
export function initProductGrid( rootEl, data = {}, options = {} ) {
	...
	const toolbarEl =
		options.toolbarEl ||
		document.querySelector( '[data-component="shop-toolbar"]' );
	...
	const render = () => {
		...
		const totalPages = Math.max(
			1,
			Math.ceil( sorted.length / state.perPage ) || 1
		);
		...
		if ( toolbarEl ) {
			const pageInput = toolbarEl.querySelector(
	...
	if ( ! rootEl.dataset.paginationBound ) {
		...
		rootEl.addEventListener(
			...
			( e ) => {
				...
				if ( toolbarEl ) {
					const topInput = toolbarEl.querySelector(
						"[data-action='page-input']"
					);
					if ( topInput ) {
		...
		rootEl.addEventListener( 'keydown', ( e ) => {
			if (
				e.target.matches( "[data-action='grid-page-input']" ) &&
				e.key === 'Enter'
			) {
				e.target.blur();
			}
