...
function renderSection( section, index ) {
	...
	return `
    <div class="legal-content__section" data-accordion-item data-item-id="${ escapeHtml(
		section.id
	) }"${ open }>
      <button type="button" class="legal-content__trigger" data-action="accordion-toggle" aria-expanded="${
			section.open ? 'true' : 'false'
		}">
        <span>${ escapeHtml( section.title ) }</span>
        <span class="legal-content__icon" aria-hidden="true"></span>
      </button>
      <div class="legal-content__panel" data-accordion-panel id="legal-panel-${ index }" ${
			section.open ? '' : 'hidden'
		}>
        ${ ( section.paragraphs || [] )
...
function buildTemplate( data ) {
    ...
    <div class="legal-content__inner l-section">
      <p class="legal-content__updated">Last updated: ${ escapeHtml(
			l.lastUpdated || ''
		) }</p>
