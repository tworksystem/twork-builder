...
function renderSlide( slide, index ) {
	...
	const titleClass =
		index === 0
			? 'shop-hero__title'
			: 'shop-hero__title shop-hero__title--duplicate';
	return `
    <article
...
function buildTemplate( data ) {
    ...
    <div class="shop-hero__wrap" role="region" aria-roledescription="carousel" aria-label="Shop promotions">
      <div class="shop-hero__track" data-list="slides">${ slides
			.map( renderSlide )
			.join( '' ) }</div>
