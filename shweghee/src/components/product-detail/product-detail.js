...
import {
	...
	renderProductCard,
	bindProductImages,
} from '../product-card/product-card.js';

function renderPrice( product ) {
	if ( product.priceRange ) {
		const [ min, max ] = product.priceRange;
		return `<p class="product-detail__price product-detail__price--range">${ formatCurrency(
			min
		) } – ${ formatCurrency( max ) }</p>`;
	}
	if ( product.salePrice != null && product.price != null ) {
		return `
      <p class="product-detail__price">
        <span class="product-detail__price--sale">${ formatCurrency(
			product.salePrice
		) }</span>
        <span class="product-detail__price--was">${ formatCurrency(
			product.price
		) }</span>
      </p>
    `;
...
function renderTabPanel( tab ) {
	const items = tab.items
		? `<ul class="product-detail__tab-list">${ tab.items
...
function buildNotFound() {
    ...
    <div class="product-detail__inner l-section">
      <div class="product-detail__not-found">
        ...
        <a class="btn btn--primary" href="${ safeUrl(
			'/pages/shop.html'
		) }">Back to Shop</a>
      </div>
    </div>
...
function buildTemplate( product, related = [] ) {
	...
	const image = imageSrc
		? `<img class="product-detail__image" src="${ imageSrc }" alt="${ escapeHtml(
	...
	const badge = product.badge?.text
		? `<span class="product-detail__badge">${ escapeHtml(
				product.badge.text
		  ) }</span>`
		: '';
	...
	const tabs = ( product.tabs || [] )
		.map( ( tab, i ) => {
			...
			return `
      <div class="product-detail__tab-panel${ active }" data-tab-panel="${ escapeHtml(
			tab.id
		) }"${ hidden }>
        ${ renderTabPanel( tab ) }
      </div>
	...
	const tabButtons = ( product.tabs || [] )
		.map( ( tab, i ) => {
			const active = i === 0 ? ' is-active' : '';
			return `<button type="button" class="product-detail__tab-btn${ active }" data-action="tab-switch" data-tab="${ escapeHtml(
				tab.id
			) }" aria-selected="${ i === 0 ? 'true' : 'false' }">${ escapeHtml(
				tab.label
			) }</button>`;
		} )
		.join( '' );
	...
	const relatedHtml = related
		.map( ( p ) => renderProductCard( p, { variant: 'compact' } ) )
		.join( '' );
    ...
    <div class="product-detail__inner l-section">
      <div class="product-detail__grid">
        ...
        <div class="product-detail__info">
          ${
				product.category
					? `<p class="product-detail__category">${ escapeHtml(
							product.category
					  ) }</p>`
					: ''
			}
          <h1 class="product-detail__title">${ escapeHtml(
				product.title
			) }</h1>
          ...
          ${
				product.sku
					? `<p class="product-detail__sku">SKU: ${ escapeHtml(
							product.sku
					  ) }</p>`
					: ''
			}
          <p class="product-detail__desc">${ escapeHtml(
				product.description || ''
			) }</p>
          ...
          <div class="product-detail__actions">
            <button class="btn btn--primary product-detail__cart-btn" type="button" data-action="add-to-cart" ${
				inStock ? '' : 'disabled'
			}>
...
export function initProductDetail( rootEl, data = {} ) {
	...
	on( rootEl, 'click', "[data-action='tab-switch']", ( _event, btn ) => {
		...
		rootEl
			.querySelectorAll( "[data-action='tab-switch']" )
			.forEach( ( b ) => {
	...
	on( rootEl, 'click', "[data-action='add-to-cart']", ( _event, btn ) => {
		...
		if ( feedback ) {
			...
			feedback.textContent =
				'Demo: Cart feature မရရှိသေးပါ။ ဖုန်းဖြင့် အော်ဒါတင်ပါ — 095-2-55122';
		}
		window.setTimeout( () => {
			btn.disabled = false;
		}, 3000 );
...
export function enrichProduct( product ) {
	const defaults = {
		description:
			'Premium Shwe Myanmar product crafted in Mandalay, Myanmar. Natural scent, natural taste (သဘာဝ အနံ့ သဘာဝ အရသာ).',
		inStock: true,
		...
		tabs: [
			{
				...
				paragraphs: [
					product.description ||
						'Shwe Myanmar Foodstuff Industry delivers consistent quality from our Mandalay production facility.',
	...
	return {
		...defaults,
		...product,
		href: product.href || `/pages/product.html?id=${ product.id }`,
	};
}
