...
function renderColumn( column ) {
	const links = ( column.links || [] )
		.map(
			( link ) =>
				`<a class="shop-footer__link" href="${ safeUrl(
					link.href
	...
	return `
    <div class="shop-footer__column" data-item-id="${ escapeHtml(
		column.id
	) }">
...
function buildTemplate( data ) {
	...
	const categoryLinks = ( f.categoryLinks || [] )
		.map(
			( label ) =>
				`<a href="#" class="shop-footer__cat-link">${ escapeHtml(
					label
				) }</a>`
		)
		.join( '' );
	const payments = ( f.payments || [] )
		.map(
			( p ) =>
				`<span class="shop-footer__pay" data-item-id="${ escapeHtml(
					p
				) }">${ escapeHtml( p ) }</span>`
		)
		.join( '' );
    ...
    <div class="shop-footer__main l-section shop-header__container">
      <div class="shop-footer__brand-block">
        <a class="shop-footer__brand" href="${ safeUrl( '/pages/home.html' ) }">
          <span>${ escapeHtml(
				f.brandPrefix || 'Farm'
			) }</span><span class="shop-footer__brand-dot">${ escapeHtml(
				f.brandSuffix || 'art.'
			) }</span>
        </a>
        ...
        ${
			f.email
				? `<p><a href="mailto:${ escapeHtml( f.email ) }">${ escapeHtml(
						f.email
				  ) }</a></p>`
				: ''
		}
      ...
      <div class="shop-footer__columns">${ ( f.columns || [] )
			.map( renderColumn )
			.join( '' ) }</div>
      <div class="shop-footer__newsletter">
        <h3 class="shop-footer__column-title">${ escapeHtml(
			f.newsletterTitle || ''
		) }</h3>
        <p class="shop-footer__newsletter-text">${ escapeHtml(
			f.newsletterText || ''
		) }</p>
        <form class="shop-footer__form" action="#" method="post" data-action="newsletter-form">
          ...
          <input id="shop-newsletter-email" type="email" name="email" placeholder="${ escapeHtml(
				f.newsletterPlaceholder || 'Email'
			) }" maxlength="120" required />
          <button type="submit" class="shop-footer__subscribe">${ escapeHtml(
...
export function initShopFooter( rootEl, data = {} ) {
	...
	rootEl
		.querySelector( "[data-action='newsletter-form']" )
