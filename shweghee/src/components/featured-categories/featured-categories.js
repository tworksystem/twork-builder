...
function renderCategoryCard( cat ) {
	const links = ( cat.sublinks || [] )
		.map( ( link ) => {
			const cls = link.highlight
				? 'featured-categories__link featured-categories__link--cta'
				: 'featured-categories__link';
			return `<a class="${ cls }" href="${ safeUrl(
				link.href
	...
	return `
    <article class="featured-categories__card" data-item-id="${ escapeHtml(
		cat.id
	) }">
      <div class="featured-categories__media">
        <img src="${ escapeHtml( cat.image || '' ) }" alt="${ escapeHtml(
			cat.title
...
function renderSlide( categories, index ) {
	...
	return `
    <div class="featured-categories__slide${ active }" data-carousel-slide aria-hidden="${
		index === 0 ? 'false' : 'true'
	}">
		...
		.map(
			( _, i ) =>
				`<button type="button" class="carousel-dots__dot${
					i === 0 ? ' is-active' : ''
				}" data-action="carousel-go" data-slide-index="${ i }" aria-label="Categories slide ${
					i + 1
				}"></button>`
		)
		.join( '' );
    ...
    <div class="shop-panel featured-categories__panel">
      <header class="shop-panel__head">
        <div class="shop-panel__title-wrap">
          ...
          <h2 class="shop-panel__title" data-field="title">${ escapeHtml(
				fc.title || ''
			) }</h2>
        </div>
        <a class="shop-panel__view-all" href="${ safeUrl(
			fc.viewAllHref
		) }">View All ›</a>
      </header>
      <div class="featured-categories__stage" role="region" aria-label="${ escapeHtml(
			fc.title || 'Featured categories'
		) }">
        ...
        <div class="featured-categories__track">${ slides
			.map( renderSlide )
			.join( '' ) }</div>
