...
function renderItem( item, index ) {
	...
	return `
    <div class="faq__item" data-accordion-item data-item-id="${ escapeHtml(
		item.id
	) }"${ open }>
      <button type="button" class="faq__trigger" data-action="accordion-toggle" aria-expanded="${
			item.open ? 'true' : 'false'
		}">
        <span class="faq__question">${ escapeHtml( item.question ) }</span>
        <span class="faq__icon" aria-hidden="true"></span>
      </button>
      <div class="faq__panel" data-accordion-panel id="faq-panel-${ index }" ${
			item.open ? '' : 'hidden'
		}>
...
function buildTemplate( data ) {
    ...
    <div class="faq__inner l-section">
      ...
      <p class="faq__contact">
        ...
        <a href="${ safeUrl( f.contactHref ) }">${ escapeHtml(
			f.contactLinkLabel || 'Contact us'
		) }</a>
      </p>
    </div>
  `;
