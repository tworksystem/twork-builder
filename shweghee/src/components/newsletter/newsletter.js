    ...
    <div class="newsletter__banner">
      <div class="newsletter__inner l-section">
        <div class="newsletter__cta">
          ...
          <p class="newsletter__title" data-field="title">${ escapeHtml(
				n.title || ''
			) }</p>
        </div>
        <form class="newsletter__form" data-action="newsletter-submit" novalidate>
          ...
          <input type="text" name="${ escapeHtml(
				n.honeypotName || 'website'
			) }" class="newsletter__hp u-hidden" tabindex="-1" autocomplete="off" aria-hidden="true" />
          <input
            id="newsletter-email"
            ...
            placeholder="${ escapeHtml(
				n.placeholder || 'Your email address'
			) }"
            required
          />
          <button class="newsletter__button btn btn--dark" type="submit">${ escapeHtml(
				n.buttonLabel || 'SUBSCRIBE'
			) }</button>
        </form>
