...
const TEMPLATE = [
    ['twork/lab-step-item', { stepNumber: '1', title: 'Book a Test', description: 'Call us or book online.' }],
    ['twork/lab-step-item', { stepNumber: '2', title: 'Sample Collection', description: 'Quick and painless collection.' }],
    ['twork/lab-step-item', { stepNumber: '3', title: 'Analysis', description: 'Processed in ISO lab.' }],
    ['twork/lab-step-item', { stepNumber: '4', title: 'Report', description: 'Sent via Email/SMS.' }],
];
...
export default function Edit({ attributes, setAttributes }) {
    ...
    const blockProps = useBlockProps({
        className: 'lab-section twork-lab-steps-section-editor lab-steps-section-editor',
        style: {
        ...
        <>
            <InspectorControls>
                <PanelBody title={__('Colors', 'twork-builder')} initialOpen={true}>
                    <PanelColorSettings
                        ...
                        colorSettings={[
                            ...
                            { value: sectionSubtitleColor, onChange: (v) => setAttributes({ sectionSubtitleColor: v ?? undefined }), label: __('Section subtitle', 'twork-builder') },
                        ]}
                    />
                    <PanelColorSettings
                        title={__('Steps', 'twork-builder')}
                        colorSettings={[
                            { value: primaryColor, onChange: (v) => setAttributes({ primaryColor: v ?? undefined }), label: __('Step number circle', 'twork-builder') },
                            { value: stepNumTextColor, onChange: (v) => setAttributes({ stepNumTextColor: v ?? undefined }), label: __('Step number text', 'twork-builder') },
                            { value: stepContentBgColor, onChange: (v) => setAttributes({ stepContentBgColor: v ?? undefined }), label: __('Step card background', 'twork-builder') },
                            { value: stepTitleColor, onChange: (v) => setAttributes({ stepTitleColor: v ?? undefined }), label: __('Step title', 'twork-builder') },
                            { value: stepDescColor, onChange: (v) => setAttributes({ stepDescColor: v ?? undefined }), label: __('Step description', 'twork-builder') },
                            { value: lineColor, onChange: (v) => setAttributes({ lineColor: v ?? undefined }), label: __('Timeline line', 'twork-builder') },
                        ]}
                    />
                </PanelBody>
                ...
                <PanelBody title={__('Container', 'twork-builder')} initialOpen={false}>
                    ...
                    <Divider />
                    <RangeControl
                        label={__('Steps area max width (px)', 'twork-builder')}
                        value={stepsContainerMaxWidth}
                        onChange={(val) => setAttributes({ stepsContainerMaxWidth: val })}
                        min={400}
                        max={1000}
                        step={10}
                        help={__('Narrower than container for timeline layout.', 'twork-builder')}
                    />
                    <RangeControl
                        label={__('Gap between steps (px)', 'twork-builder')}
                        value={stepsGap}
                        onChange={(val) => setAttributes({ stepsGap: val })}
                        min={20}
                        max={80}
                        step={5}
                    />
                    ...
                    {animationOnScroll && (
                        <>
                            <SelectControl
                                ...
                                value={animationType}
                                options={[
                                    { label: __('Stagger up', 'twork-builder'), value: 'stagger-up' },
            ...
            <div {...blockProps}>
                <div className="lab-container" style={containerStyle}>
                    ...
                    <div className="lab-steps-container" style={stepsContainerStyle}>
                        <InnerBlocks
