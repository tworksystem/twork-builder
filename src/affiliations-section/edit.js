...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <>
            <InspectorControls>
                    ...
                    {showSectionTitle && (
                        <>
                            ...
                            <SelectControl
                                ...
                                onChange={(val) => setAttributes({ sectionTitleTextTransform: val })}
                            />
                            <RangeControl
                                label={__('Letter Spacing (px)', 'twork-builder')}
                                value={sectionTitleLetterSpacing}
                                onChange={(val) => setAttributes({ sectionTitleLetterSpacing: val })}
                                min={0}
                                max={8}
                                step={1}
                            />
                ...
                <PanelBody title={__('Layout Settings', 'twork-builder')} initialOpen={false}>
                    <SelectControl
                        ...
                        value={layoutMode}
                        options={[
                            { label: __('Grid', 'twork-builder'), value: 'grid' },
                            { label: __('Slider', 'twork-builder'), value: 'slider' }
                        ]}
                        onChange={(val) => setAttributes({ layoutMode: val })}
                        help={__('Grid: static layout. Slider: horizontal carousel with optional autoplay.', 'twork-builder')}
                    />
                    {!isSlider && (
                        <>
                            <RangeControl
                                label={__('Min Logo Width (px)', 'twork-builder')}
                                value={gridMinWidth}
                                onChange={(val) => setAttributes({ gridMinWidth: val })}
                                min={80}
                                max={300}
                                step={10}
                                help={__('Minimum width per grid cell', 'twork-builder')}
                            />
                            <RangeControl
                                label={__('Gap Between Logos (px)', 'twork-builder')}
                                value={gap}
                    ...
                    {isSlider && (
                        <>
                            ...
                            />
                            <RangeControl
                                label={__('Autoplay speed (ms)', 'twork-builder')}
                                value={sliderSpeed}
                                onChange={(val) => setAttributes({ sliderSpeed: val })}
                                min={2000}
                                max={10000}
                                step={500}
                                help={sliderAutoplay ? undefined : __('Used when autoplay is on.', 'twork-builder')}
                            />
                            <ToggleControl
                                label={__('Show prev/next arrows', 'twork-builder')}
                                checked={sliderShowArrows}
                                onChange={(val) => setAttributes({ sliderShowArrows: val })}
                            />
                            <ToggleControl
                                label={__('Show dots', 'twork-builder')}
                                checked={sliderShowDots}
                                onChange={(val) => setAttributes({ sliderShowDots: val })}
                            />
                            <RangeControl
                                label={__('Slides to show (desktop)', 'twork-builder')}
                                value={sliderSlidesToShow}
                                onChange={(val) => setAttributes({ sliderSlidesToShow: val })}
                                min={1}
                                max={8}
                                step={1}
                            />
                            <RangeControl
                                label={__('Slides to show (tablet)', 'twork-builder')}
                                value={sliderSlidesToShowTablet}
                                onChange={(val) => setAttributes({ sliderSlidesToShowTablet: val })}
                                min={1}
                                max={5}
                                step={1}
                            />
                            <RangeControl
                                label={__('Slides to show (mobile)', 'twork-builder')}
                                value={sliderSlidesToShowMobile}
                                onChange={(val) => setAttributes({ sliderSlidesToShowMobile: val })}
                                min={1}
                                max={3}
                                step={1}
                            />
                            <RangeControl
                                label={__('Gap between slides (px)', 'twork-builder')}
                                value={sliderGap}
                                onChange={(val) => setAttributes({ sliderGap: val })}
                                min={8}
                                max={48}
                                step={4}
                            />
                    ...
                    <RangeControl
                        label={__('Logo Max Width (px)', 'twork-builder')}
                        value={logoMaxWidth}
                    ...
                    />
                    <RangeControl
                        label={__('Logo Max Height (px)', 'twork-builder')}
                        value={logoMaxHeight}
                        onChange={(val) => setAttributes({ logoMaxHeight: val })}
                        min={40}
                        max={200}
                        step={5}
                        help={__('Max height for all logos in this section. Leave room for aspect ratio.', 'twork-builder')}
                    />
                </PanelBody>

                <PanelBody title={__('Logo Styling', 'twork-builder')} initialOpen={true}>
                    <RangeControl
                        label={__('Logo Opacity', 'twork-builder')}
                        value={logoOpacity}
                        onChange={(val) => setAttributes({ logoOpacity: val })}
                        min={0.3}
                        max={1}
                        step={0.1}
                    />
                    <ToggleControl
                        label={__('Show logos in grayscale', 'twork-builder')}
                        checked={logoGrayscale}
                        onChange={(val) => setAttributes({ logoGrayscale: val })}
                        help={__('When off, logos display in full color from the start. When on, grayscale with color on hover.', 'twork-builder')}
                    />
                    <Divider />
                    <RangeControl
                        label={__('Hover Opacity', 'twork-builder')}
                        value={logoHoverOpacity}
                        onChange={(val) => setAttributes({ logoHoverOpacity: val })}
                        min={0.8}
                        max={1}
                        step={0.05}
                    />
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ logoHoverScale: val })}
                        min={1}
                        max={1.3}
                        step={0.05}
                    />
            ...
            <section {...blockProps} data-animation={animationOnScroll} data-animation-type={animationType}>
                <div className="jivaka-container" style={containerStyle}>
                    {showSectionTitle && (
                        <RichText
                            ...
                            value={sectionTitle}
                            onChange={(val) => setAttributes({ sectionTitle: val })}
                            placeholder={__('Our Affiliations', 'twork-builder')}
                    ...
                    {isSlider ? (
                        ...
                        >
                            ...
                            <p style={{ marginTop: 8, fontSize: 12, color: '#666' }}>
                                {__('Slider: arrows/dots and autoplay work on the front-end.', 'twork-builder')}
                            </p>
                        </div>
                    ) : (
                        <div
                            className={`logo-grid${animationOnScroll ? ' fade-up' : ''} twork-affiliations-grid`}
                            style={{
                                ...gridStyle,
                                '--logo-max-width': `${logoMaxWidth}px`,
                                '--logo-max-height': `${logoMaxHeight}px`,
