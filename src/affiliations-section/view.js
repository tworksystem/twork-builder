...
(function () {
    ...
    function initScrollAnimation() {
        if (!('IntersectionObserver' in window)) {
            document.querySelectorAll('.twork-affiliations-section .logo-grid.fade-up').forEach(function (el) {
                el.classList.add('is-visible');
            });
            return;
        }
        var sections = document.querySelectorAll('.twork-affiliations-section[data-animation="true"]');
    ...
    function initOneSlider(sliderEl) {
        ...
        var slides = Array.prototype.filter.call(track.children, function (node) {
            return node.nodeType === 1 && !node.classList.contains('block-list-appender');
        });
        var total = slides.length;
        if (total === 0) return;

        var section = sliderEl.closest('.twork-affiliations-section');
        var gapPx = parseInt(section ? section.getAttribute('data-slider-gap') || '' : '24', 10) || 24;
        var container = sliderEl;
        var containerWidth = container.offsetWidth;
        var slidesToShow = getSlidesToShow(container);
        var slideWidth = (containerWidth - gapPx * (slidesToShow - 1)) / slidesToShow;
        // Prevent extreme gap values from breaking layout (negative or tiny widths)
        if (!isFinite(slideWidth) || slideWidth < 40) {
            slideWidth = Math.max(40, containerWidth / Math.max(1, slidesToShow));
        }
        ...
        var speed = parseInt(sliderEl.getAttribute('data-slider-speed') || 4000, 10) || 4000;
        var autoplay = sliderEl.getAttribute('data-slider-autoplay') !== 'false';
        ...
        function buildDots() {
            ...
            for (var i = 0; i <= maxIndex; i++) {
                ...
                btn.type = 'button';
                btn.className = 'logo-slider-dot' + (i === 0 ? ' is-active' : '');
        ...
        function resetAutoplay() {
            ...
            if (autoplay && sliderEl.getAttribute('data-slider-autoplay') !== 'false') {
        ...
        if (typeof ResizeObserver !== 'undefined') {
            resizeObserver = new ResizeObserver(function () {
                ...
                slideWidth = (containerWidth - gapPx * (slidesToShow - 1)) / slidesToShow;
                if (!isFinite(slideWidth) || slideWidth < 40) {
                    slideWidth = Math.max(40, containerWidth / Math.max(1, slidesToShow));
                }
    ...
    function initSliders() {
        var sections = document.querySelectorAll('.twork-affiliations-section[data-layout="slider"]');
        sections.forEach(function (section) {
    ...
    window.TworkAffiliationsSection = { init: init, initSliders: initSliders, version: '1.1.0' };
})();
