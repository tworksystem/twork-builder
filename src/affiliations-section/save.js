...
export default function save({ attributes }) {
    ...
    const blockProps = useBlockProps.save({
        className: `twork-affiliations-section affiliations-section section-padding${isSlider ? ' has-slider' : ''}`,
        style: {
            backgroundColor,
    ...
    const titleMarginTop = showSectionTitle && sectionTitle ? 0 : titleMarginBottom;
            ...
            <div className="jivaka-container" style={containerStyle}>
                ...
                {isSlider ? (
                    ...
                    >
                        ...
                        {sliderShowArrows && (
                            <>
                                <button type="button" className="logo-slider-prev" aria-label="Previous" />
                                <button type="button" className="logo-slider-next" aria-label="Next" />
                            </>
                        )}
                        {sliderShowDots && <div className="logo-slider-dots" aria-hidden="true" />}
                    </div>
                ) : (
                    <div
                        className={`logo-grid${animationOnScroll ? ' fade-up' : ''}`}
                        style={{
                            display: 'grid',
