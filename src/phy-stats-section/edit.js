...
const TEMPLATE = [
    ...
    ['twork/phy-stat-card', { statNumber: '10+', label: 'Expert Therapists' }],
        ...
        <>
            <InspectorControls>
                <PanelBody title={__('Section', 'twork-builder')} initialOpen={true}>
                    ...
                    <RangeControl
                        ...
                        max={50}
                        step={5}
                        help={__('Negative values pull the stats up (e.g. over hero). Default -50.', 'twork-builder')}
                    />
                </PanelBody>
                ...
                <PanelBody title={__('Grid', 'twork-builder')} initialOpen={true}>
                    <RangeControl
                ...
                <PanelBody title={__('Card style', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
                        title={__('Accent (number & border)', 'twork-builder')}
                        colorSettings={[
                            {
                                value: primaryColor,
                    ...
                    />
                    <RangeControl
                        label={__('Bottom border width (px)', 'twork-builder')}
                        value={cardBorderBottomWidth}
                        onChange={(val) => setAttributes({ cardBorderBottomWidth: val })}
                        min={0}
                        max={20}
                        step={1}
                    />
                ...
                <PanelBody title={__('Animation', 'twork-builder')} initialOpen={false}>
                    <ToggleControl
                        label={__('Stagger animation on scroll', 'twork-builder')}
                        checked={animationOnScroll}
                        onChange={(val) => setAttributes({ animationOnScroll: val })}
                        help={__('Adds stagger-up class for scroll-triggered animation.', 'twork-builder')}
                    />
                </PanelBody>
            </InspectorControls>
            ...
            <div {...blockProps}>
                <div className="phy-container" style={containerStyle}>
                    <div className="phy-stats-grid phy-stats-grid-editor" style={gridStyle}>
                        <InnerBlocks
