        ...
        <>
            <InspectorControls>
                ...
                >
                    ...
                    />
                    <TextControl
                        label={__('Department "All" option', 'twork-builder')}
                        value={attrs.departmentAllLabel}
                        onChange={(val) => setAttributes({ departmentAllLabel: val })}
                    />
                    <TextControl
                        label={__('Gender label', 'twork-builder')}
                        value={attrs.genderLabel}
                        onChange={(val) => setAttributes({ genderLabel: val })}
                    />
                    <TextControl
                        label={__('Gender "All" option', 'twork-builder')}
                        value={attrs.genderAllLabel}
                        onChange={(val) => setAttributes({ genderAllLabel: val })}
                    />
                    <TextControl
                        label={__('Reset button text', 'twork-builder')}
                        value={attrs.resetButtonText}
                        onChange={(val) => setAttributes({ resetButtonText: val })}
                    />
                </PanelBody>
                ...
                >
                    ...
                    />
                    <RangeControl
                        label={__('Section margin top (px)', 'twork-builder')}
                        value={attrs.sectionMarginTop}
                        onChange={(val) => setAttributes({ sectionMarginTop: val })}
                        min={-150}
                        max={50}
                        step={5}
                        help={__('Negative values pull the filter up over the hero. Default -80.', 'twork-builder')}
                    />
                    <RangeControl
                ...
                >
                    ...
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ boxBorderTopWidth: val })}
                        min={0}
                        max={12}
                        step={1}
                    />
                    <PanelColorSettings
                        ...
                        colorSettings={[
                            ...
                            },
                            {
                                ...
                                onChange: (val) => setAttributes({ boxBorderTopColor: val }),
                    ...
                    <PanelColorSettings
                        ...
                        colorSettings={[
                            ...
                            },
                            {
                                ...
                                onChange: (val) => setAttributes({ inputFocusBorderColor: val }),
                        ...
                        colorSettings={[
                            {
                                ...
                                onChange: (val) => setAttributes({ resetButtonBg: val }),
                                label: __('Background', 'twork-builder')
                            },
                            {
                                value: attrs.resetButtonColor,
                                onChange: (val) => setAttributes({ resetButtonColor: val }),
                                label: __('Text', 'twork-builder')
                            },
                            {
                                value: attrs.resetButtonHoverBg,
                                onChange: (val) => setAttributes({ resetButtonHoverBg: val }),
                                label: __('Hover background', 'twork-builder')
                            },
                            {
                                value: attrs.resetButtonHoverColor,
                                onChange: (val) => setAttributes({ resetButtonHoverColor: val }),
                ...
                >
                    <ToggleControl
                        label={__('Add animate-hero class', 'twork-builder')}
                        checked={attrs.addAnimationClass}
                        onChange={(val) => setAttributes({ addAnimationClass: val })}
                        help={__('Adds class for hero entrance animation if your theme uses it.', 'twork-builder')}
                    />
                </PanelBody>
            </InspectorControls>
            ...
            <div {...blockProps}>
                <div style={containerStyle}>
                    <div
                        ...
                        style={{ marginTop: `${sectionMarginTop}px`, marginBottom: `${sectionMarginBottom}px` }}
                    >
                        <div className="search-box" style={searchBoxStyle}>
                            <div className="filter-group" style={{ flex: 1, minWidth: 200, display: 'flex', flexDirection: 'column', gap: 8 }}>
                            ...
                            <div className="filter-group" style={{ flex: 1, minWidth: 200, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                ...
                                <select className="filter-input" disabled style={inputStyle} aria-hidden>
                                    <option>{departmentAllLabel}</option>
                                </select>
                            </div>
                            <div className="filter-group" style={{ flex: 1, minWidth: 200, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                <label style={labelStyle}>{genderLabel}</label>
                                <select className="filter-input" disabled style={inputStyle} aria-hidden>
                                    <option>{genderAllLabel}</option>
                                </select>
                            </div>
                            <div className="search-btn-wrapper" style={{ alignSelf: 'flex-end' }}>
                                <button type="button" className="btn-reset" disabled style={{ padding: '12px 20px', borderRadius: 5, fontWeight: 700, cursor: 'default', background: attrs.resetButtonBg, color: attrs.resetButtonColor, border: 'none' }}>
                                    <i className="fas fa-undo" aria-hidden /> {resetButtonText}
                                </button>
                            </div>
                        </div>
                    </div>
