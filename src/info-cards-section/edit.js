...
export default function Edit({ attributes = {}, setAttributes }) {
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={__('Header & Description', 'twork-builder')} initialOpen={true}>
                    <ToggleControl
                        label={__('Show section header', 'twork-builder')}
                        help={__('Display a title above the cards.', 'twork-builder')}
                        checked={showSectionHeader}
                        onChange={(val) => setAttributes({ showSectionHeader: val })}
                    />
                    <ToggleControl
                        label={__('Show section description', 'twork-builder')}
                        help={__('Display a short description above the cards.', 'twork-builder')}
                        checked={showSectionDescription}
                        onChange={(val) => setAttributes({ showSectionDescription: val })}
                    />
                    <Divider />
                    <SelectControl
                        label={__('Header & description alignment', 'twork-builder')}
                        help={__('Align title and description text above the cards.', 'twork-builder')}
                ...
                <PanelBody title={__('Appearance', 'twork-builder')} initialOpen={true}>
                    {PanelColorSettings && (
                        <PanelColorSettings
                            title={__('Section Colors', 'twork-builder')}
                            colorSettings={[
                                {
                                    value: backgroundColor ?? DEFAULT_ATTRS.backgroundColor,
                                    onChange: (val) => setAttributes({ backgroundColor: val }),
                                    label: __('Section Background', 'twork-builder')
                                },
                                {
                                    value: iconColor ?? accentColor ?? DEFAULT_ATTRS.iconColor,
                                    onChange: (val) => setAttributes({ iconColor: val }),
                                    label: __('Icon Color', 'twork-builder')
                                },
                                {
                                    value: iconBackground ?? DEFAULT_ATTRS.iconBackground,
                                    onChange: (val) => setAttributes({ iconBackground: val }),
                                    label: __('Icon Background', 'twork-builder')
                                },
                                {
                                    value: accentColor ?? DEFAULT_ATTRS.accentColor,
                                    onChange: (val) => setAttributes({ accentColor: val }),
                                    label: __('CTA Link Default', 'twork-builder')
                                }
                            ]}
                        />
                    )}
                ...
                <PanelBody title={__('Animation', 'twork-builder')} initialOpen={false}>
                    <ToggleControl
                        label={__('Enable Stagger Animation', 'twork-builder')}
                    ...
                    {animationOnScroll && (
                        <>
                            ...
                            <select
                                value={animationType}
                                onChange={(event) => setAttributes({ animationType: event.target.value })}
                                style={{ width: '100%', marginTop: '10px' }}
                            >
                                <option value="fadeInUp">{__('Fade In Up', 'twork-builder')}</option>
                                <option value="fadeIn">{__('Fade In', 'twork-builder')}</option>
                                <option value="zoomIn">{__('Zoom In', 'twork-builder')}</option>
                            </select>
                        </>
                    )}
                </PanelBody>
            ...
            <div {...blockProps}>
                ...
                >
                    ...
                    <div className="info-grid info-grid-editor" style={gridStyle}>
                        <InnerBlocks
