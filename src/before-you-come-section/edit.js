...
const TEMPLATE = [
    ['twork/before-you-come-item', { icon: 'fas fa-clock', title: 'Fasting', description: 'Please fast for at least 8-10 hours before the checkup. Water is allowed.' }],
    ['twork/before-you-come-item', { icon: 'fas fa-file-medical', title: 'Reports', description: 'Bring any previous medical records and current medication list.' }],
    ['twork/before-you-come-item', { icon: 'fas fa-calendar-check', title: 'Appointment', description: 'Booking is recommended to avoid long waiting times.' }]
];
...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={__('Section Spacing', 'twork-builder')} initialOpen={true}>
                    <RangeControl
                        ...
                        max={200}
                        step={5}
                        help={__('Space above the section (gap from the block above).', 'twork-builder')}
                    />
                    <RangeControl
                        ...
                        max={200}
                        step={5}
                        help={__('Space below the section.', 'twork-builder')}
                    />
                </PanelBody>
                ...
                <PanelBody title={__('Section Title', 'twork-builder')} initialOpen={true}>
                    ...
                    <ToggleControl
                        label={__('Show Section Description', 'twork-builder')}
                        checked={showSectionDescription}
                        onChange={(val) => setAttributes({ showSectionDescription: val })}
                        help={__('Optional text under the main title.', 'twork-builder')}
                    />
                    {showSectionDescription && (
                        <>
                            <PanelColorSettings
                            ...
                            />
                            <SelectControl
                                label={__('Description Alignment', 'twork-builder')}
                                ...
                                onChange={(val) => setAttributes({ sectionDescriptionAlignment: val })}
                            />
                            <RangeControl
                                label={__('Gap above description (px)', 'twork-builder')}
                                value={sectionDescriptionMarginTop}
                                onChange={(val) => setAttributes({ sectionDescriptionMarginTop: val })}
                                min={0}
                                max={80}
                                step={5}
                                help={__('Space between title and description.', 'twork-builder')}
                            />
                            <RangeControl
                                label={__('Gap below description (px)', 'twork-builder')}
                                value={sectionDescriptionMarginBottom}
                                onChange={(val) => setAttributes({ sectionDescriptionMarginBottom: val })}
                                min={0}
                                max={80}
                                step={5}
                                help={__('Space below description (before cards).', 'twork-builder')}
                            />
                        </>
                    )}
                </PanelBody>
                ...
                <PanelBody title={__('Grid & Cards', 'twork-builder')} initialOpen={false}>
                    <RangeControl
                        ...
                        max={500}
                        step={10}
                        help={__('Grid uses auto-fit with this min width. Smaller = more columns.', 'twork-builder')}
                    />
                    <RangeControl
                        ...
                        colorSettings={[
                            {
                                ...
                                label: __('Card Background Color', 'twork-builder')
                            }
                        ]}
                    />
                    <RangeControl
                ...
                <div style={containerStyle}>
                    ...
                    {showSectionDescription && (
                        <RichText
                            ...
                            placeholder={__('Add a short description under the title...', 'twork-builder')}
