...
const DEFAULT_SPEC_ITEMS = [
    { id: 1, iconClass: 'fas fa-graduation-cap', text: 'MBBS, M.Med.Sc, MRCP (UK)' },
...
export default function Edit({ attributes, setAttributes }) {
    ...
    const safeSpecItems = Array.isArray(specItems) ? specItems : DEFAULT_SPEC_ITEMS;
    ...
    const addSpecItem = () => {
        const newId = safeSpecItems.length ? Math.max(...safeSpecItems.map(s => s.id)) + 1 : 1;
        setAttributes({
            specItems: [...safeSpecItems, { id: newId, iconClass: 'fas fa-check', text: __('New item', 'twork-builder') }]
        });
    };
    ...
    const removeSpecItem = (id) => {
        setAttributes({ specItems: safeSpecItems.filter(item => item.id !== id) });
    };
            ...
            <InspectorControls>
                ...
                >
                            ...
                            colorSettings={[
                                {
                                    ...
                                    onChange: (val) => setAttributes({ backgroundGradientStart: val }),
                                    label: __('Start Color', 'twork-builder')
                                },
                                {
                                    value: backgroundGradientEnd,
                                    onChange: (val) => setAttributes({ backgroundGradientEnd: val }),
                    ...
                    <RangeControl
                        ...
                        value={typeof paddingTop === 'number' ? paddingTop : 60}
                    ...
                    <RangeControl
                        ...
                        value={typeof paddingBottom === 'number' ? paddingBottom : 60}
                ...
                >
                    <RangeControl
                        ...
                        value={typeof containerMaxWidth === 'number' ? containerMaxWidth : 1200}
                    ...
                    <RangeControl
                        ...
                        value={typeof containerPadding === 'number' ? containerPadding : 20}
                ...
                >
                    {!profileImage ? (
                        <MediaPlaceholder
                            onSelect={(media) => setAttributes({
                                ...
                                profileImageAlt: profileImageAlt || media.alt || ''
                            })}
                            allowedTypes={['image']}
                            multiple={false}
                            labels={{ title: __('Profile Image', 'twork-builder') }}
                        />
                    ) : (
                        <div>
                            <img
                            ...
                            <Button
                                isSecondary
                                isSmall
                                onClick={() => setAttributes({ profileImage: '', profileImageId: null })}
                            >
                    ...
                    <TextControl
                        ...
                        onChange={(val) => setAttributes({ profileImageAlt: val })}
                        help={__('Accessibility description for the image', 'twork-builder')}
                    />
                    {Divider && <Divider />}
                    <ToggleControl
                    ...
                    {showBadge && (
                        <>
                            ...
                            />
                            <TextControl
                                label={__('Badge Icon (Font Awesome class)', 'twork-builder')}
                                value={badgeIcon ?? ''}
                                onChange={(val) => setAttributes({ badgeIcon: val })}
                                help={__('e.g. fas fa-check-circle', 'twork-builder')}
                            />
                        </>
                    )}
                </PanelBody>
                ...
                <PanelBody
                    ...
                    initialOpen={false}
                >
                    <BaseControl label={__('Specification items (qualifications, experience, languages)', 'twork-builder')}>
                        {safeSpecItems.map((item, index) => (
                            ...
                            >
                                <TextControl
                                    ...
                                    onChange={(val) => updateSpecItem(item.id, 'iconClass', val)}
                                    help={__('e.g. fas fa-graduation-cap', 'twork-builder')}
                                />
                                <TextControl
                                    label={__('Text', 'twork-builder')}
                                    value={item.text ?? ''}
                                    onChange={(val) => updateSpecItem(item.id, 'text', val)}
                                />
                                <Button
                                    isDestructive
                                    isSmall
                ...
                >
                    <TextControl
                        ...
                        value={profileBio ?? ''}
                        onChange={(val) => setAttributes({ profileBio: val })}
                        help={__('Short biography / description', 'twork-builder')}
                    />
                </PanelBody>
                ...
                <PanelBody
                    ...
                    initialOpen={false}
                >
                    <BaseControl label={__('Primary Button (e.g. Book Appointment)', 'twork-builder')}>
                        <TextControl
                            label={__('Text', 'twork-builder')}
                        ...
                        <ToggleControl
                            ...
                            onChange={(val) => setAttributes({ primaryButtonTarget: val })}
                        />
                    </BaseControl>
                    {Divider && <Divider />}
                    <BaseControl label={__('Secondary Button (e.g. Share Profile)', 'twork-builder')}>
                        <TextControl
                            label={__('Text', 'twork-builder')}
                        ...
                        <ToggleControl
                            ...
                            onChange={(val) => setAttributes({ secondaryButtonTarget: val })}
                        />
                    </BaseControl>
                </PanelBody>
            </InspectorControls>
            ...
            <div {...blockProps}>
                <div style={containerStyle}>
                    <div className="profile-grid" style={gridStyle}>
                        <div className="profile-img-box" style={{
                            position: 'relative',
                            borderRadius: '10px',
                            overflow: 'hidden',
                            ...
                            {profileImage ? (
                                <img
                                    src={profileImage}
                                    alt={str(profileImageAlt) || str(doctorName)}
                                    decoding="async"
                                ...
                                <div style={{
                                    ...
                                    {__('Add profile image', 'twork-builder')}
                                </div>
                            )}
                            {showBadge && (
                                <span className="profile-badge" style={{
                                    ...
                                    <i className={str(badgeIcon)} aria-hidden="true" /> {str(badgeText)}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="profile-info">
                            ...
                            <RichText
                                ...
                                onChange={(val) => setAttributes({ designation: val })}
                                placeholder={__('Designation...', 'twork-builder')}
                            ...
                            {safeSpecItems.length > 0 && (
                                <div className="profile-specs" style={{
                                    display: 'flex',
                                    flexWrap: 'wrap',
                                    gap: '15px',
                                    marginBottom: '30px'
                                    ...
                                    {safeSpecItems.map((item, index) => (
                                        ...
                                        >
                                            <i className={str(item.iconClass)} style={{ color: '#f48b2a' }} aria-hidden="true" />
                                            {str(item.text)}
                                        </div>
                                    ))}
                                </div>
                            ...
                            <RichText
                                ...
                                onChange={(val) => setAttributes({ profileBio: val })}
                                placeholder={__('Profile bio...', 'twork-builder')}
