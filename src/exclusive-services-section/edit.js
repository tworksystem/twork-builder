...
export default function Edit({ attributes, setAttributes }) {
    ...
    const TEMPLATE = [
        ['twork/exclusive-service-item', { icon: 'fas fa-language', title: 'Language Interpretation', description: 'Our team speaks English, Chinese, and Thai to ensure clear communication.' }],
        ['twork/exclusive-service-item', { icon: 'fas fa-plane-arrival', title: 'Airport Transfer', description: 'Complimentary pickup and drop-off services for patients and attendants.' }],
        ['twork/exclusive-service-item', { icon: 'fas fa-passport', title: 'Visa Assistance', description: 'Provision of official documents required for Medical Visa application.' }],
        ['twork/exclusive-service-item', { icon: 'fas fa-hotel', title: 'Accommodation', description: 'Partnerships with nearby hotels for comfortable stays for family members.' }],
        ['twork/exclusive-service-item', { icon: 'fas fa-utensils', title: 'International Cuisine', description: 'Customized meal plans catering to diverse cultural and dietary preferences.' }],
        ['twork/exclusive-service-item', { icon: 'fas fa-wifi', title: 'Connectivity', description: 'Free local SIM cards and high-speed Wi-Fi to keep you connected with home.' }]
    ];
            ...
            <InspectorControls>
                ...
                <PanelBody title={__('Layout Settings', 'twork-builder')} initialOpen={false}>
                    <RangeControl
                        ...
                        max={6}
                        step={1}
                        help={__('Number of columns on desktop (used when min-width allows)', 'twork-builder')}
                    />
                    <RangeControl
                    ...
                    <RangeControl
                        ...
                        max={400}
                        step={10}
                        help={__('Minimum width for auto-fill grid', 'twork-builder')}
                    />
                    <Divider />
                    <RangeControl
                ...
                <PanelBody title={__('Hover & Animation', 'twork-builder')} initialOpen={false}>
                    <ToggleControl
                        ...
                        help={__('Slide right on hover (translateX)', 'twork-builder')}
                    />
                    {hoverEffect && (
                        <RangeControl
                            label={__('Hover Translate X (px)', 'twork-builder')}
                            value={hoverTranslateX}
                            onChange={(val) => setAttributes({ hoverTranslateX: val })}
                            min={0}
                            max={20}
                            step={1}
                            help={__('Horizontal movement on hover', 'twork-builder')}
                        />
                    )}
                    <Divider />
                    <ToggleControl
