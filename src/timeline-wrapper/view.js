...
( function () {
	...
	function initItemAnimations( timeline ) {
		...
		const animationEnabled =
			wrapper.getAttribute( 'data-animation' ) === 'true';
		...
		const animationType =
			wrapper.getAttribute( 'data-animation-type' ) || 'fadeInUp';
		const animationDelay =
			parseInt( wrapper.getAttribute( 'data-animation-delay' ) ) || 100;
	...
	function processTimeline( timeline ) {
		...
		if ( items.length === 0 ) {
			const altItems = timeline.querySelectorAll(
				'[class*="timeline-item"], [data-type="twork/timeline-item"]'
			);
