  ...
  return (
    <>
      <InspectorControls>
        ...
        >
          <RangeControl
            label={__('Vertical padding (px)', 'twork-builder')}
            value={paddingY}
        ...
        >
          <RichText
            ...
            placeholder={__("Have a Doctor's Prescription?", 'twork-builder')}
          />
          <RichText
            tagName="p"
            value={description}
          ...
          <ToggleControl
            label={__('Show checklist items', 'twork-builder')}
            checked={showChecklist}
            onChange={(val) => setAttributes({ showChecklist: val })}
          />

          {showChecklist && (
            <>
              {(Array.isArray(checklistItems) ? checklistItems : []).map(
                (item, index) => (
                  <div
                    key={index}
                    style={{
                  ...
                  >
                    <TextControl
                      label={sprintf(
                        ...
                        __('Item %d', 'twork-builder'),
                        index + 1
                      )}
                      value={item}
                      onChange={(val) => updateChecklistItem(index, val)}
                    />
                    <button
                      type="button"
        ...
        >
          ...
          />
          <TextControl
            label={__('Button URL (optional)', 'twork-builder')}
            help={__(
        ...
        >
          {imageUrl ? (
            ...
            <MediaPlaceholder
              ...
              allowedTypes={['image']}
              labels={{
                title: __('Prescription image', 'twork-builder'),
              }}
            />
          )}
        </PanelBody>
          ...
          <div className="ph-upload-section fade-up">
            <div className="ph-upload-grid">
              <div className="ph-upload-content">
                ...
                {showChecklist && Array.isArray(checklistItems) && (
                  <ul
                    style={{
                      listStyle: 'none',
                      padding: 0,
