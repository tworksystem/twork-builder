...
( function () {
	...
	function initScrollAnimations() {
		...
		storySections.forEach( ( section ) => {
			const animationType =
				section.getAttribute( 'data-animation-type' ) || 'fadeInUp';
			const baseDelay =
				parseInt( section.getAttribute( 'data-animation-delay' ) ) ||
				100;
			...
			const observer = new IntersectionObserver( ( entries ) => {
				entries.forEach( ( entry ) => {
					if (
						...
						const index = Array.from( storyItems ).indexOf(
							entry.target
						);
	...
	function initCounterAnimations() {
		const counters = document.querySelectorAll(
			'.story-item .counter[data-target]'
		);
