...
export default function Edit({ attributes, setAttributes }) {
    ...
    const blockProps = useBlockProps({
        className: 'twork-before-you-come-item-editor before-you-come-item-card',
        style: {
            borderRadius: '8px',
        ...
        <>
            <InspectorControls>
                <PanelBody title={__('Icon', 'twork-builder')} initialOpen={true}>
                    <TextControl
                        label={__('Font Awesome Icon Class', 'twork-builder')}
                        value={icon}
                        onChange={(val) => setAttributes({ icon: val })}
                        help={__('e.g. fas fa-clock, fas fa-file-medical, fas fa-calendar-check', 'twork-builder')}
                    />
                </PanelBody>
                ...
                <PanelBody title={__('Description', 'twork-builder')} initialOpen={false}>
                    ...
                    <RangeControl
                        ...
                        max={60}
                        step={2}
                        help={__('Space below the description.', 'twork-builder')}
                    />
                </PanelBody>
            </InspectorControls>
            ...
            <div {...blockProps}>
                ...
                >
                    <i className={icon} aria-hidden style={{ flexShrink: 0, fontSize: '1.1em' }} />
                    <RichText
                        tagName="h4"
                        value={title}
