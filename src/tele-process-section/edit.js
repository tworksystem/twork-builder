...
const TEMPLATE = [
    ['twork/tele-process-card', { stepNumber: 1, iconClass: 'fas fa-user-md', title: 'Choose Doctor', description: 'Select a specialist from our list and view their available online schedules.' }],
    ['twork/tele-process-card', { stepNumber: 2, iconClass: 'fas fa-calendar-check', title: 'Book Slot', description: 'Pick a time that works for you and make a secure online payment.' }],
    ['twork/tele-process-card', { stepNumber: 3, iconClass: 'fas fa-link', title: 'Get Link', description: 'Receive a secure encrypted video meeting link via SMS or Email.' }],
    ['twork/tele-process-card', { stepNumber: 4, iconClass: 'fas fa-file-prescription', title: 'Consultation', description: 'See the doctor and get your digital prescription instantly in the app.' }]
];
    ...
    return (
        <>
            <InspectorControls>
                        ...
                        onChange={(val) =>
                            setAttributes(
                                val
                                    ...
                                    : { showSectionTitle: false, sectionSubtitle: '' }
                            )
                        }
                        help={__('Display a title and optional description above the process cards.', 'twork-builder')}
                    />
                    {(showSectionTitle || sectionSubtitle) && (
                        <>
                            <p style={{ margin: '0 0 12px 0', fontSize: '12px', color: '#757575' }}>
                                {__('Click the title or description in the preview to edit directly.', 'twork-builder')}
                            </p>
                            <TextControl
                                label={__('Title (or edit in preview)', 'twork-builder')}
                                value={sectionTitle}
                                onChange={(val) => setAttributes({ sectionTitle: val })}
                                placeholder={__('How It Works', 'twork-builder')}
                            />
                            <TextControl
                                label={__('Description (or edit in preview)', 'twork-builder')}
                                value={sectionSubtitle}
                                onChange={(val) => setAttributes({ sectionSubtitle: val })}
                                placeholder={__('Optional intro text…', 'twork-builder')}
                                help={__('Optional. You can also click "Add description…" in the preview to type.', 'twork-builder')}
                            />
                            <SelectControl
                            ...
                            <ToggleControl
                                ...
                                onChange={(val) => setAttributes({ useHeaderFadeUp: val })}
                            />
                        </>
                    )}
                </PanelBody>
                <PanelBody title={__('Title & Description styling', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
                        title={__('Title color', 'twork-builder')}
                        colorSettings={[
                            {
                                value: sectionTitleColor,
                                onChange: (val) => setAttributes({ sectionTitleColor: val || '#212121' }),
                        ...
                        colorSettings={[
                            {
                                ...
                                onChange: (val) => setAttributes({ sectionSubtitleColor: val || '#555555' }),
                ...
                <PanelBody title={__('Section', 'twork-builder')} initialOpen={false}>
                    <TextControl
                        label={__('Section anchor ID', 'twork-builder')}
                        value={sectionAnchor}
                        onChange={(val) => setAttributes({ sectionAnchor: val || 'how-it-works' })}
                        help={__('Used for #how-it-works link. Leave empty to use block anchor.', 'twork-builder')}
                    />
                    <PanelColorSettings
                        ...
                        colorSettings={[
                            {
                                ...
                                onChange: (val) => setAttributes({ backgroundColor: val || '#ffffff' }),
                ...
                <PanelBody title={__('Grid', 'twork-builder')} initialOpen={false}>
                    ...
                    <RangeControl
                        ...
                        value={minItemWidth}
                        onChange={(val) => setAttributes({ minItemWidth: val })}
                        min={200}
                        max={400}
                        step={10}
                        help={__('Used for auto-fit grid: minmax(minItemWidth, 1fr)', 'twork-builder')}
                    />
                </PanelBody>
            ...
            <section {...blockProps} id={sectionAnchor || undefined}>
                <div className="jivaka-container" style={containerStyle}>
                    {(showSectionTitle || sectionSubtitle) && (
                        <div
                            className={`section-header process-section-header ${useHeaderFadeUp ? 'fade-up' : ''}`}
                            style={headerStyle}
                        >
                            {showSectionTitle && (
                                <RichText
                            ...
                            <RichText
                                ...
                                value={sectionSubtitle}
                                onChange={(val) => setAttributes({ sectionSubtitle: val })}
                                placeholder={__('Add description…', 'twork-builder')}
