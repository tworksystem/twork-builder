...
export default function Edit({ attributes, setAttributes, clientId }) {
    ...
    const cardStyleOptions = [
        { label: __('Primary (Orange Gradient)', 'twork-builder'), value: 'primary' },
    ...
    const updateInfoBlock = (index, field, value) => {
        ...
        newInfoBlocks[index] = { ...newInfoBlocks[index], [field]: value };
        setAttributes({ infoBlocks: newInfoBlocks });
    };
            ...
            <InspectorControls>
                ...
                >
                    <SelectControl
                        ...
                        onChange={(val) => setAttributes({ itemType: val })}
                        help={__('Choose between service card or info block', 'twork-builder')}
                    />
                </PanelBody>
                ...
                {itemType === 'card' && (
                    <>
                        ...
                        >
                            <SelectControl
                                label={__('Style Preset', 'twork-builder')}
                                value={cardStyle}
                            ...
                            {cardStyle === 'solid' && (
                                <PanelColorSettings
                                    ...
                                    colorSettings={[
                                        {
                                            value: customBgColor,
                                            onChange: (val) => setAttributes({ customBgColor: val }),
                                            label: __('Background Color', 'twork-builder')
                                        },
                                        {
                                            value: customTextColor,
                                            onChange: (val) => setAttributes({ customTextColor: val }),
                            ...
                            {(cardStyle === 'custom' || cardStyle === 'solid') && (
                                <PanelColorSettings
                        ...
                        <PanelBody
                            title={__('Typography Settings', 'twork-builder')}
                            initialOpen={false}
                        >
                            <BaseControl label={__('Meta Text', 'twork-builder')}>
                                <RangeControl
                                ...
                                <SelectControl
                                    ...
                                    onChange={(val) => setAttributes({ metaTextTransform: val })}
                                />
                                <PanelColorSettings
                                    colorSettings={[
                                        {
                            ...
                            <BaseControl label={__('Description', 'twork-builder')}>
                                <RangeControl
                                    label={__('Font Size (rem)', 'twork-builder')}
                ...
                {itemType === 'info' && (
                    <PanelBody
                        ...
                        initialOpen={true}
                    >
                        <p>{__('Add information blocks with icons', 'twork-builder')}</p>
                        ...
                        {infoBlocks.map((block, index) => (
                            <div key={index} style={{ 
                                ...
                                <BaseControl label={__(`Info Block ${index + 1}`, 'twork-builder')}>
                                    <TextControl
                                        label={__('Icon Class', 'twork-builder')}
                                        value={block.icon}
                                        onChange={(val) => updateInfoBlock(index, 'icon', val)}
                                        help={__('e.g., fas fa-phone', 'twork-builder')}
                                    />
                                    <TextControl
                                        label={__('Title', 'twork-builder')}
                                        value={block.title}
                                        onChange={(val) => updateInfoBlock(index, 'title', val)}
                                    />
                                    <TextControl
                                        label={__('Content', 'twork-builder')}
                                        value={block.content}
                                        onChange={(val) => updateInfoBlock(index, 'content', val)}
                                    />
                                    <TextControl
                                        label={__('Subtitle (Optional)', 'twork-builder')}
                                        value={block.subtitle || ''}
                                        onChange={(val) => updateInfoBlock(index, 'subtitle', val)}
                                    />
                                    <Button
                                        isDestructive
                    ...
                    <>
                        ...
                        >
                            <RichText
                                ...
                                className="meta"
                                value={metaText}
                                onChange={(val) => setAttributes({ metaText: val })}
                                placeholder={__('Meta text...', 'twork-builder')}
                                style={{
                    ...
                    >
                        {infoBlocks.length === 0 ? (
                            <p style={{ textAlign: 'center', opacity: 0.7 }}>
                                {__('Add info blocks using the sidebar settings', 'twork-builder')}
                            </p>
                        ) : (
                            infoBlocks.map((block, index) => (
                                <div key={index} className="info-block" style={{
                                    display: 'flex',
