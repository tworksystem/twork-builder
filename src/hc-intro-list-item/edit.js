...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <>
            <PanelBody title={__('List Item', 'twork-builder')} initialOpen={false}>
                <TextControl
                    label={__('Icon class (Font Awesome)', 'twork-builder')}
                    value={iconClass || ''}
                    onChange={(val) => setAttributes({ iconClass: val || 'fas fa-check-circle' })}
                    help={__('e.g. fas fa-check-circle, fas fa-check', 'twork-builder')}
                />
            </PanelBody>
            <li {...blockProps}>
