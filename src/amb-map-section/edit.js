...
export default function Edit({ attributes, setAttributes }) {
    ...
    const addTownship = () => {
        const id = townships.length ? Math.max(...townships.map((t) => t.id)) + 1 : 1;
        setAttributes({
            townships: [...townships, { id, text: __('New area', 'twork-builder') }]
        });
    };

    const updateTownship = (id, text) => {
        setAttributes({
            townships: townships.map((t) => (t.id === id ? { ...t, text } : t))
        });
    };

    const removeTownship = (id) => {
        setAttributes({ townships: townships.filter((t) => t.id !== id) });
    };
    ...
    return (
        <>
            <InspectorControls>
                    ...
                    {showIcon && (
                        <>
                            <TextControl
                                ...
                                value={icon}
                                onChange={(val) => setAttributes({ icon: val || 'fa-map-marked-alt' })}
                                help={__('e.g. fa-map-marked-alt', 'twork-builder')}
                            />
                            <PanelColorSettings
                ...
                <PanelBody title={__('Title & Description', 'twork-builder')} initialOpen={true}>
                    <PanelColorSettings
                ...
                <PanelBody title={__('Townships / Areas', 'twork-builder')} initialOpen={true}>
                    <BaseControl label={__('Location tags', 'twork-builder')}>
                        {townships.map((t) => (
                            <div
                                key={t.id}
                                style={{
                            ...
                            >
                                <TextControl
                                    value={t.text}
                                    onChange={(val) => updateTownship(t.id, val)}
                                    style={{ flex: 1 }}
                                />
                                <Button
                                    isDestructive
                    ...
                    <RangeControl
                        ...
                        value={townshipsGap}
                        onChange={(val) => setAttributes({ townshipsGap: val })}
                        min={5}
                        max={30}
                        step={5}
                    />
                    <RangeControl
                        label={__('Tags margin top (px)', 'twork-builder')}
                        value={townshipsMarginTop}
                        onChange={(val) => setAttributes({ townshipsMarginTop: val })}
                        min={0}
                        max={60}
                        step={5}
                    />
                ...
                <PanelBody title={__('Township tag style', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
                        title={__('Tag colors', 'twork-builder')}
                        colorSettings={[
                            {
                                value: townshipBgColor,
                                onChange: (val) => setAttributes({ townshipBgColor: val }),
                                label: __('Background', 'twork-builder')
                            },
                            {
                                value: townshipTextColor,
                                onChange: (val) => setAttributes({ townshipTextColor: val }),
                    ...
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ townshipPaddingVertical: val })}
                        min={5}
                        max={25}
                        step={1}
                    />
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ townshipPaddingHorizontal: val })}
                        min={10}
                        max={50}
                        step={5}
                    />
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ townshipBorderRadius: val })}
                        min={0}
                        max={50}
                        step={5}
                    />
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ townshipFontWeight: val })}
                        min={400}
                        max={900}
                        step={100}
                    />
                ...
                <PanelBody title={__('Box style', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
            ...
            <div {...blockProps}>
                <div style={containerStyle}>
                    ...
                    >
                        {__('Ambulance Map / Coverage Section', 'twork-builder')}
                    </div>

                    <div className="amb-map-box twork-amb-map-box" style={boxStyle}>
                        {showIcon && icon && (
                            <i
