...
(function () {
    ...
    function initCsrCtaSection() {
        if (!('IntersectionObserver' in window)) {
            const containers = document.querySelectorAll('.twork-csr-cta-section .jivaka-container.fade-up');
            containers.forEach((el) => el.classList.add('is-visible'));
            return;
        }

        const sections = document.querySelectorAll('.twork-csr-cta-section[data-animation="true"]');
        ...
        sections.forEach((section) => {
            const container = section.querySelector('.jivaka-container.fade-up');
