...
export default function Edit({ attributes, setAttributes }) {
    const { logoUrl, logoId, logoAlt, placeholderColor, linkUrl, linkTarget, linkRel } = attributes;

    const blockProps = useBlockProps({
        className: 'twork-csr-partner-logo-item-editor partner-logo-wrap csr-partner-logo-wrap',
        style: {
            display: 'flex',
            alignItems: 'center',
    ...
    return (
        <>
            <InspectorControls>
                    ...
                    {!logoUrl ? (
                        ...
                        <div>
                            <img
                                src={logoUrl}
                                alt=""
                                style={{ width: '100%', maxWidth: 120, height: 'auto', marginBottom: '10px' }}
                            />
                            <TextControl
                                label={__('Alt Text', 'twork-builder')}
                                value={logoAlt}
                                onChange={(val) => setAttributes({ logoAlt: val })}
                                help={__('For accessibility (e.g., Red Cross)', 'twork-builder')}
                            />
                            <Button
                                isSecondary
                                isSmall
                ...
                <PanelBody title={__('Placeholder (when no logo)', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
            ...
            <div {...blockProps}>
                {!logoUrl ? (
                    <span className="csr-partner-placeholder" aria-hidden="true" />
                ) : linkUrl ? (
                    <a
                        href={linkUrl}
                        target={linkTarget}
