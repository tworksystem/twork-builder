...
const TEMPLATE = [
    ['twork/health-check-step-item', { icon: 'fas fa-ban', title: '1. Fasting', description: 'Do not eat for 8-10 hours before the test. Water is allowed.' }],
    ['twork/health-check-step-item', { icon: 'fas fa-calendar-check', title: '2. Appointment', description: 'Book your slot in advance to avoid waiting.' }],
    ['twork/health-check-step-item', { icon: 'fas fa-vial', title: '3. Sample', description: 'Provide blood/urine samples and undergo scans.' }],
    ['twork/health-check-step-item', { icon: 'fas fa-file-medical-alt', title: '4. Review', description: 'Consult with the doctor to understand your report.' }],
];
        ...
        <>
            <InspectorControls>
                    ...
                    {showSectionTitle && (
                        <>
                            <PanelColorSettings
                                ...
                                colorSettings={[
                                    { value: sectionTitleColor, onChange: (v) => setAttributes({ sectionTitleColor: v }), label: __('Title Color', 'twork-builder') },
                                ]}
                            />
                            <RangeControl
                            ...
                            <RangeControl
                                ...
                                onChange={(v) => setAttributes({ sectionTitleFontWeight: v })}
                                min={100}
                                max={900}
                                step={100}
                            />
                            <SelectControl
                                ...
                                onChange={(v) => setAttributes({ sectionTitleAlignment: v })}
                            />
                            <RangeControl
                                label={__('Header Margin Bottom (px)', 'twork-builder')}
                                value={headerMarginBottom}
                                onChange={(v) => setAttributes({ headerMarginBottom: v })}
                                min={20}
                                max={80}
                                step={5}
                            />
                ...
                <PanelBody title={__('Layout', 'twork-builder')} initialOpen={true}>
                    <RangeControl
                        ...
                        max={6}
                        step={1}
                        help={__('Number of step columns on desktop.', 'twork-builder')}
                    />
                    <RangeControl
                ...
                <PanelBody title={__('Colors', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
                        ...
                        colorSettings={[
                            { value: backgroundColor, onChange: (v) => setAttributes({ backgroundColor: v }), label: __('Section background', 'twork-builder') },
                            { value: iconColor, onChange: (v) => setAttributes({ iconColor: v }), label: __('Icon color', 'twork-builder') },
                            { value: titleColor, onChange: (v) => setAttributes({ titleColor: v }), label: __('Step title color', 'twork-builder') },
                            { value: descriptionColor, onChange: (v) => setAttributes({ descriptionColor: v }), label: __('Step description color', 'twork-builder') },
                            { value: connectorLineColor, onChange: (v) => setAttributes({ connectorLineColor: v }), label: __('Connector line (between steps)', 'twork-builder') },
                        ]}
                    />
                </PanelBody>
            ...
            <section {...blockProps}>
                <div className="chk-container" style={containerStyle}>
                    ...
                    >
                        {__('Health Check Steps Section (Editor View)', 'twork-builder')}
                    </div>
                    {showSectionTitle && (
                        <div
                        ...
                        >
                            <RichText
                                ...
                                placeholder={__('How to Prepare', 'twork-builder')}
                                style={{
                    ...
                    <div
                        ...
                        style={{ ...gridStyle, marginTop: showSectionTitle ? undefined : `${headerMarginBottom}px` }}
