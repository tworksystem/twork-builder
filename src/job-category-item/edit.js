...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={__('Category Title', 'twork-builder')} initialOpen={true}>
                    <PanelColorSettings
                        title={__('Title Color', 'twork-builder')}
                        colorSettings={[
                            { value: categoryTitleColor, onChange: (val) => setAttributes({ categoryTitleColor: val }), label: __('Title Color', 'twork-builder') },
                            { value: categoryBorderColor, onChange: (val) => setAttributes({ categoryBorderColor: val }), label: __('Left Border Color', 'twork-builder') }
                        ]}
                    />
                    <RangeControl
                        label={__('Font Size (rem)', 'twork-builder')}
                        value={categoryTitleFontSize}
                        onChange={(val) => setAttributes({ categoryTitleFontSize: val })}
                        min={1.2}
                        max={2}
                        step={0.1}
                    />
                    <RangeControl
                        ...
                        onChange={(val) => setAttributes({ categoryMarginBottom: val })}
                        min={30}
                        max={100}
                        step={5}
                    />
