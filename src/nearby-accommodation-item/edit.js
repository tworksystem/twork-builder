        ...
        <>
            <InspectorControls>
                <PanelBody title={__('Image', 'twork-builder')} initialOpen={true}>
                    <BaseControl label={__('Hotel Image', 'twork-builder')}>
                        {!image ? (
                            <MediaPlaceholder
                                onSelect={(media) => setAttributes({ image: media.url, imageId: media.id })}
                                allowedTypes={['image']}
                                multiple={false}
                                labels={{ title: __('Hotel Image', 'twork-builder') }}
                            />
                        ) : (
                            <div>
                                <img
                ...
                <PanelBody title={__('Subtitle', 'twork-builder')} initialOpen={false}>
                    <TextControl
                        label={__('Subtitle', 'twork-builder')}
                        value={subtitle}
                        onChange={(val) => setAttributes({ subtitle: val })}
                        help={__('e.g. 5 Star • 5 mins away', 'twork-builder')}
                    />
                    <PanelColorSettings
                ...
                <PanelBody title={__('Link', 'twork-builder')} initialOpen={false}>
                    <BaseControl label={__('Link URL', 'twork-builder')} help={__('Make the whole card clickable. Leave empty for no link.', 'twork-builder')}>
                        <URLInput
                            value={linkUrl}
                    ...
                    {linkUrl && (
                        <ToggleControl
                            ...
                            onChange={(val) => setAttributes({ linkOpenInNewTab: val })}
                        />
                    )}
                </PanelBody>
                ...
                <PanelBody title={__('Card Layout', 'twork-builder')} initialOpen={false}>
                    ...
                    />
                    <RangeControl
                        label={__('Overlay Padding (px)', 'twork-builder')}
                        value={overlayPadding}
                        onChange={(val) => setAttributes({ overlayPadding: val })}
                        min={10}
                        max={40}
                        step={5}
                    />
                    ...
                    (() => {
                        const cardContent = (
                            <>
                                ...
                                />
                                <div className="hotel-overlay" style={overlayStyle}>
                                    <RichText
                                        tagName="h4"
                                        value={title}
                                        onChange={(val) => setAttributes({ title: val })}
                                        placeholder={__('Hotel name...', 'twork-builder')}
                                        style={{
                                            margin: 0,
                                    ...
                                    <RichText
                                        ...
                                        value={subtitle}
                                        onChange={(val) => setAttributes({ subtitle: val })}
                                        placeholder={__('5 Star • 5 mins away', 'twork-builder')}
                                        style={{
                                            margin: '5px 0 0',
