...
export default function Edit({ attributes, setAttributes }) {
    ...
    const blockProps = useBlockProps({
        className: 'twork-health-check-checklist-editor chk-section chk-checklist-section',
        style: {
    ...
    const addItem = () => setAttributes({ checklistItems: [...items, __('New checklist item', 'twork-builder')] });
    const updateItem = (index, value) => {
        const next = [...items];
        next[index] = value;
        setAttributes({ checklistItems: next });
    };
    const removeItem = (index) => setAttributes({ checklistItems: items.filter((_, i) => i !== index) });
    ...
    return (
        <>
            <InspectorControls>
                ...
                <PanelBody title={__('Checklist Items', 'twork-builder')} initialOpen={true}>
                    {items.map((item, index) => (
                        ...
                        >
                            <TextControl
                                ...
                                placeholder={__('Checklist item text', 'twork-builder')}
                                style={{ flex: 1 }}
                            />
                            <Button
                                isDestructive
                    ...
                    <Button isSecondary isSmall onClick={addItem} icon="plus">
                        {__('Add checklist item', 'twork-builder')}
                    </Button>
                </PanelBody>

                <PanelBody title={__('Right Column Image', 'twork-builder')} initialOpen={false}>
                            ...
                            <MediaPlaceholder
                                onSelect={(media) =>
                                    setAttributes({
                                        imageUrl: media.url,
                                        imageId: media.id,
                                        imageAlt: media.alt || __('Doctor', 'twork-builder'),
                                    })
                                }
                                allowedTypes={['image']}
                                multiple={false}
                ...
                <PanelBody title={__('Container & Layout', 'twork-builder')} initialOpen={false}>
                    ...
                    <RangeControl
                        ...
                        max={80}
                        step={5}
                        help={__('Space between checklist column and image', 'twork-builder')}
                    />
                    <RangeControl
                        label={__('Checklist Box Border Radius (px)', 'twork-builder')}
                    ...
                    />
                    <RangeControl
                        label={__('Checklist Box Padding (px)', 'twork-builder')}
                        value={boxPadding}
                ...
                <PanelBody title={__('Checklist Styling', 'twork-builder')} initialOpen={false}>
                    <PanelColorSettings
                        title={__('Checkbox Accent Color', 'twork-builder')}
                        colorSettings={[
                            {
                                value: checkboxAccentColor,
                                onChange: (val) => setAttributes({ checkboxAccentColor: val }),
                                label: __('Checkbox accent (focus/check)', 'twork-builder'),
                            },
                        ]}
                    />
                </PanelBody>
                ...
                <PanelBody title={__('Animation Settings', 'twork-builder')} initialOpen={false}>
                    <ToggleControl
                        ...
                        help={__('Animate columns when they scroll into view', 'twork-builder')}
                    />
                    {animationOnScroll && (
                        <>
                            <SelectControl
            ...
            <section {...blockProps}>
                ...
                <div className="chk-container" style={containerStyle}>
                    ...
                    >
                        {__('Health Check Checklist Section (Editor View)', 'twork-builder')}
                    </div>
                                ...
                                >
                                    {showSectionTitle && (
                                        <RichText
                                            ...
                                            placeholder={__('Are You Due for a Checkup?', 'twork-builder')}
                                            style={{
                                                fontSize: `${sectionTitleFontSize}rem`,
                                                fontWeight: sectionTitleFontWeight,
                                                color: sectionTitleColor,
                                                marginBottom:
                                                    showSectionSubtitle ? `${sectionTitleMarginBottom}px` : 0,
                                                marginTop: 0,
                                            }}
                                        />
                                    )}
                            ...
                            >
                                {items.length > 0 ? (
                                    ...
                                    <p style={{ color: '#999', margin: 0 }}>
                                        {__('Add checklist items in the sidebar.', 'twork-builder')}
                                    </p>
                                )}
                            </div>
                        </div>
