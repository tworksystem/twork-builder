...
( function () {
	...
	const initPackagesSection = () => {
		...
		sections.forEach( ( section ) => {
			...
			const cards = section
				? section.querySelectorAll( '.package-card' )
				: [];
			...
			const hoverTranslateY =
				parseFloat( section.dataset.hoverTranslateY ) || -10;
			...
			if ( hoverEnabled && cards.length ) {
				cards.forEach( ( card ) => {
					...
					card.addEventListener( 'mouseleave', function () {
						const recommended =
							this.classList.contains( 'recommended' );
						this.style.transform = recommended
							? 'scale(1.02)'
							: 'translateY(0)';
					} );
				} );
					...
					( entries ) => {
						entries.forEach( ( entry ) => {
							if ( entry.isIntersecting ) {
								...
								entry.target.style.transform = recommended
									? 'scale(1.02)'
									: 'translateY(0)';
