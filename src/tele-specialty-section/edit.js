...
const TEMPLATE = [
    ['twork/tele-specialty-item', { iconClass: 'fas fa-stethoscope', title: 'General Medicine' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-allergies', title: 'Dermatology' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-baby', title: 'Pediatrics' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-brain', title: 'Psychology' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-female', title: 'Gynecology' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-carrot', title: 'Nutrition / Diet' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-heartbeat', title: 'Cardiology Follow-up' }],
    ['twork/tele-specialty-item', { iconClass: 'fas fa-notes-medical', title: 'Report Review' }]
];
        ...
        <>
            <InspectorControls>
                <PanelBody title={__('Content', 'twork-builder')} initialOpen={true}>
                    ...
                    <Divider />
                    <ToggleControl
                        label={__('Add fade-up class (section header)', 'twork-builder')}
                        checked={useFadeUp}
                ...
                <PanelBody title={__('Grid', 'twork-builder')} initialOpen={false}>
                    ...
                    />
                    <RangeControl
                        label={__('Min Item Width (px)', 'twork-builder')}
                        value={minItemWidth}
                        onChange={(val) => setAttributes({ minItemWidth: val })}
                        min={160}
                        max={320}
                        step={10}
                        help={__('Used for auto-fill grid: minmax(minItemWidth, 1fr)', 'twork-builder')}
                    />
                </PanelBody>
            ...
            <section {...blockProps}>
                <div className="jivaka-container" style={containerStyle}>
                    {(showSectionTitle || sectionSubtitle) && (
                        <div
                            className={`section-header ${useFadeUp ? 'fade-up' : ''}`}
                            style={headerStyle}
                        >
                            {showSectionTitle && (
                                <RichText
                                    ...
                                    value={sectionTitle}
                                    onChange={(val) => setAttributes({ sectionTitle: val })}
                                    placeholder={__('Available for Tele-Consultation', 'twork-builder')}
                                    style={{
                            ...
                            {sectionSubtitle && (
                                <RichText
                                    ...
                                    value={sectionSubtitle}
                                    onChange={(val) => setAttributes({ sectionSubtitle: val })}
                                    placeholder={__('Not all conditions require…', 'twork-builder')}
                                    style={{
