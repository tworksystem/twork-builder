...
export default function Edit( { attributes, setAttributes, isSelected } ) {
	...
	const blockProps = useStableBlockProps(
		...
		[ sectionMarginBottom, sectionMarginTop, sectionPaddingBottom ]
	);
	...
	const addScheduleRow = () => {
		const newId = scheduleRows.length
			? Math.max( ...scheduleRows.map( ( r ) => r.id ) ) + 1
			: 1;
		setAttributes( {
			scheduleRows: [
				...scheduleRows,
	...
	const addExperienceItem = () => {
		const newId = experienceItems.length
			? Math.max( ...experienceItems.map( ( e ) => e.id ) ) + 1
			: 1;
		setAttributes( {
			experienceItems: [
				...experienceItems,
	...
	const removeExperienceItem = ( id ) => {
		setAttributes( {
			experienceItems: experienceItems.filter( ( e ) => e.id !== id ),
		} );
	};
				...
				<InspectorControls>
					...
					>
						<TextControl
							...
							onChange={ ( val ) =>
								setAttributes( { bookingSidebarId: val } )
							}
							help={ __(
						...
						<TextControl
							...
							onChange={ ( val ) =>
								setAttributes( { consultationFeeLabel: val } )
							}
						/>
						...
						<TextControl
							...
							onChange={ ( val ) =>
								setAttributes( { consultationFeeAmount: val } )
							}
						/>
					...
					<div className="profile-grid" style={ gridStyle }>
						<div className="main-content animate-item">
							...
							<div
								...
								className={
									'tab-content' +
									( activeTab === tab1Id ? ' active' : '' )
								}
								role="tabpanel"
								aria-labelledby={ `tab-${ tab1Id }-editor` }
								style={ {
									display:
										activeTab === tab1Id ? 'block' : 'none',
									marginBottom: 30,
								} }
							>
							...
							<div
								...
								className={
									'tab-content' +
									( activeTab === tab2Id ? ' active' : '' )
								}
								role="tabpanel"
								aria-labelledby={ `tab-${ tab2Id }-editor` }
								style={ {
									display:
										activeTab === tab2Id ? 'block' : 'none',
									marginBottom: 20,
								} }
							>
								<h3 style={ { marginBottom: 20 } }>
						...
						>
							<h3 style={ { color: '#f48b2a', marginTop: 0 } }>
								{ bookingTitle }
							</h3>
