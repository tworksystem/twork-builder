...
export default function Edit({ attributes, setAttributes }) {
  ...
  const updateTabFeature = (tabIndex, featureIndex, value) => {
    ...
    const existingFeatures =
      newTabs[tabIndex]?.features && Array.isArray(newTabs[tabIndex].features)
  ...
  const removeTab = (index) => {
    ...
    if (!localActiveTab || !newTabs.some((t) => t.id === localActiveTab)) {
      ...
      <InspectorControls>
        ...
        >
          <TextControl
            ...
            onChange={(val) => setAttributes({ sectionBackground: val })}
          />
          <TextControl
        ...
        >
          {tabs && tabs.length > 0 ? (
            tabs.map((tab, index) => (
              ...
              >
                <TextControl
                  label={__('Tab ID (slug)', 'twork-builder')}
                  help={__(
                    'Used for linking button to content. Keep unique and lowercase.',
                    'twork-builder'
                  )}
                  value={tab.id}
                  onChange={(val) => updateTabField(index, 'id', val || '')}
                />
                <TextControl
                  label={__('Button Label', 'twork-builder')}
                  value={tab.label}
                  onChange={(val) => updateTabField(index, 'label', val)}
                />
                <TextControl
                  ...
                  value={tab.iconClass}
                  onChange={(val) => updateTabField(index, 'iconClass', val)}
                />
                <TextControl
                  label={__('Pane Title', 'twork-builder')}
                  value={tab.paneTitle}
                  onChange={(val) => updateTabField(index, 'paneTitle', val)}
                />
                <TextControl
                  label={__('Short Description', 'twork-builder')}
                  value={tab.description}
                  onChange={(val) =>
                    updateTabField(index, 'description', val)
                  }
                />

                <BaseControl
                  label={__('Pane Image', 'twork-builder')}
                  help={__(
                ...
                >
                  <MediaPlaceholder
                    onSelect={(media) => {
                      if (!media) return;
                      updateTabField(index, 'imageUrl', media.url || '');
                      updateTabField(index, 'imageId', media.id || 0);
                      updateTabField(
                        index,
                        'imageAlt',
                    ...
                    onSelectURL={(url) =>
                      updateTabField(index, 'imageUrl', url || '')
                    }
                    accept="image/*"
                    allowedTypes={['image']}
                    mediaId={tab.imageId}
                ...
                <BaseControl
                  label={__('Feature List Items', 'twork-builder')}
                  help={__(
                  ...
                  {(tab.features && tab.features.length
                    ...
                    <TextControl
                      ...
                      label={sprintf(
                        ...
                        __('Feature %d', 'twork-builder'),
                        featureIndex + 1
                      )}
                      value={feature}
                      onChange={(val) =>
                        updateTabFeature(index, featureIndex, val)
                      }
                    />
                  ))}
                </BaseControl>
                ...
                <div style={{ marginTop: '10px' }}>
                  ...
                  >
                    {__('Remove this tab', 'twork-builder')}
                  </Button>
                </div>
              </BaseControl>
            ))
        ...
        <div className="rad-container">
          <div className="rad-header fade-up">
            <RichText
              ...
              placeholder={__('Diagnostic Services', 'twork-builder')}
            />
            <RichText
              tagName="p"
              ...
              {tabs &&
                tabs.map((tab) => {
                  const featuresToShow =
                    tab.features && tab.features.length
                      ? tab.features.filter((item) => item && item.trim() !== '')
                      : [];
