...
(function () {
    ...
    function initCsrPartnersSection() {
        if (!('IntersectionObserver' in window)) {
            const grids = document.querySelectorAll('.twork-csr-partners-section .logo-grid.fade-up');
            grids.forEach((el) => el.classList.add('is-visible'));
            return;
        }

        const sections = document.querySelectorAll('.twork-csr-partners-section[data-animation="true"]');
    ...
    window.TworkCsrPartnersSection = { init: initCsrPartnersSection, version: '1.0.0' };
})();
