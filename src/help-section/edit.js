...
const DEFAULT_OPTION = { value: '', label: __('Select Department', 'twork-builder') };
...
export default function Edit({ attributes, setAttributes }) {
    ...
    const addSelectOption = () => {
        ...
        next.push({ value: `opt-${Date.now()}`, label: __('New option', 'twork-builder') });
    ...
    const removeSelectOption = (index) => {
        ...
        setAttributes({ selectOptions: next.length ? next : [DEFAULT_OPTION] });
    };

    const options = Array.isArray(selectOptions) && selectOptions.length
        ? selectOptions
        : [DEFAULT_OPTION];
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={__('Left column (Intro)', 'twork-builder')} initialOpen={true}>
                    <TextControl
                        label={__('Meta title', 'twork-builder')}
                        value={metaTitle}
                        onChange={(v) => setAttributes({ metaTitle: v })}
                    />
                    <TextControl
                        label={__('Title line 1', 'twork-builder')}
                        value={mainTitleLine1}
                        onChange={(v) => setAttributes({ mainTitleLine1: v })}
                    />
                    <TextControl
                        label={__('Title highlight word', 'twork-builder')}
                        value={mainTitleHighlight}
                        onChange={(v) => setAttributes({ mainTitleHighlight: v })}
                    />
                    <TextControl
                        label={__('Title line 2', 'twork-builder')}
                        value={mainTitleLine2}
                        onChange={(v) => setAttributes({ mainTitleLine2: v })}
                    />
                    <TextControl
                    ...
                    />
                    <TextControl
                        label={__('Sub text (with icon)', 'twork-builder')}
                        value={subText}
                        onChange={(v) => setAttributes({ subText: v })}
                    />
                    <TextControl
                        label={__('Icon class (e.g. fas fa-mobile-alt)', 'twork-builder')}
                        value={iconClass}
                        onChange={(v) => setAttributes({ iconClass: v })}
                    />
                </PanelBody>

                <PanelBody title={__('Booking card', 'twork-builder')} initialOpen={true}>
                    <TextControl
                        label={__('Card meta title', 'twork-builder')}
                        value={bookingMetaTitle}
                        onChange={(v) => setAttributes({ bookingMetaTitle: v })}
                    />
                    <TextControl
                        label={__('Card title', 'twork-builder')}
                        value={bookingTitle}
                        onChange={(v) => setAttributes({ bookingTitle: v })}
                    />
                    <TextControl
                        label={__('Card description', 'twork-builder')}
                        value={bookingDescription}
                        onChange={(v) => setAttributes({ bookingDescription: v })}
                        multiline
                        rows={3}
                    />
                    <Divider />
                    <BaseControl
                        label={__('Select dropdown options', 'twork-builder')}
                        help={__('First option is used as placeholder. Value is submitted; label is shown.', 'twork-builder')}
                    >
                        {(options || []).map((opt, i) => (
                            <div
                                key={i}
                            ...
                            >
                                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <TextControl
                                        ...
                                        value={opt?.value ?? ''}
                                        onChange={(v) => updateSelectOption(i, 'value', v)}
                                        style={{ flex: 1 }}
                                    />
                                    <Button
                                        isDestructive
                                        ...
                                        icon="no-alt"
                                        label={__('Remove option', 'twork-builder')}
                                    />
                                </div>
                                <TextControl
                                    label={__('Label', 'twork-builder')}
                                    value={opt?.label ?? ''}
                                    onChange={(v) => updateSelectOption(i, 'label', v)}
                                />
                            </div>
                        ))}
                        <Button isSecondary isSmall onClick={addSelectOption} style={{ marginTop: 8 }}>
                            {__('Add option', 'twork-builder')}
                        </Button>
                    </BaseControl>
                    <Divider />
                    <TextControl
                        label={__('Select name attribute', 'twork-builder')}
                        value={selectName}
                        onChange={(v) => setAttributes({ selectName: v })}
                    />
                    <TextControl
                        label={__('Select aria-label', 'twork-builder')}
                        value={selectAriaLabel}
                        onChange={(v) => setAttributes({ selectAriaLabel: v })}
                    />
                    <PanelColorSettings
                        ...
                        colorSettings={[
                            {
                                ...
                                onChange: (val) => setAttributes({ bookingTitleColor: val }),
                                label: __('Booking card title', 'twork-builder'),
                            },
                        ]}
                    />
                </PanelBody>
                        ...
                        colorSettings={[
                            {
                                ...
                                label: __('Highlight word color', 'twork-builder'),
                            },
                            {
                                value: cardBgColor,
            ...
            <section {...blockProps}>
                <div className="jivaka-container" style={containerStyle}>
                    <div className="help-content-grid" style={gridStyle}>
                        <div className="help-text-content" style={leftColumnStyle}>
                            <RichText
                                tagName="p"
                                className="meta-title"
                                value={metaTitle}
                                onChange={(v) => setAttributes({ metaTitle: v })}
                                placeholder={__('Meta title…', 'twork-builder')}
                            />
                            <h2 style={{ marginTop: 0, marginBottom: 25, fontSize: '3.5rem', fontWeight: 300, lineHeight: 1.1 }}>
                                <RichText
                                    tagName="span"
                                    value={mainTitleLine1}
                                    onChange={(v) => setAttributes({ mainTitleLine1: v })}
                                ...
                                <RichText
                                    ...
                                    style={{ color: highlightColor, fontWeight: 900 }}
                                />
                                {' '}
                                <RichText
                                    tagName="span"
                            ...
                            <RichText
                                ...
                                value={mainDescription}
                                onChange={(v) => setAttributes({ mainDescription: v })}
                                placeholder={__('Main description…', 'twork-builder')}
                            />
                            <p style={{ fontSize: '0.9rem', marginTop: 10 }}>
                                <i className={iconClass} style={{ color: highlightColor }} aria-hidden />
                                {' '}
                                <RichText
                                    tagName="span"
                                    value={subText}
                                    onChange={(v) => setAttributes({ subText: v })}
                                    placeholder={__('Sub text…', 'twork-builder')}
                                />
                            </p>
                        </div>

                        <div className="help-booking-card" style={{ ...cardStyle, ...rightColumnStyle }}>
                            <RichText
                                tagName="p"
                                className="meta-title"
                                value={bookingMetaTitle}
                                onChange={(v) => setAttributes({ bookingMetaTitle: v })}
                                placeholder={__('Card meta title…', 'twork-builder')}
                            />
                            <RichText
                                tagName="h3"
                                value={bookingTitle}
                                ...
                                style={{ color: bookingTitleColor, marginTop: 0, marginBottom: 15 }}
                            />
                            <RichText
                                tagName="p"
                                value={bookingDescription}
                            ...
                            <div className="custom-select-wrapper">
                                <select
                                    ...
                                    disabled
                                    style={{ width: '100%', padding: '15px 20px', borderRadius: 30 }}
                                >
                                    {(options || []).map((opt, i) => (
