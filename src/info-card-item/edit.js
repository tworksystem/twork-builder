...
const DASHICON_OPTIONS = [
    ...
    { value: 'dashicons-admin-generic', label: __('Generic / Cog', 'twork-builder') },
    ...
    { value: 'dashicons-format-status', label: __('Chat / Status', 'twork-builder') },
...
function IconPreview({ iconType, iconClass, dashicon, iconImageUrl }) {
    ...
    if (iconType === 'dashicon' && dashicon) {
        return <span className={`dashicons ${dashicon}`} aria-hidden="true" />;
    }
    if (iconClass) {
        ...
        <>
            <InspectorControls>
                ...
                <PanelBody title={__('Icon', 'twork-builder')} initialOpen={true}>
                    ...
                    {iconType === 'fontawesome' && (
                        <TextControl
                            ...
                            help={__('e.g. fas fa-phone-volume, fas fa-envelope-open-text', 'twork-builder')}
                        />
                    )}

                    {iconType === 'dashicon' && (
                        <SelectControl
                            label={__('WordPress icon (Dashicon)', 'twork-builder')}
                            value={dashicon}
                            options={DASHICON_OPTIONS}
                            onChange={(val) => setAttributes({ dashicon: val })}
                        />
                    )}
                            ...
                            <MediaUploadCheck>
                                <MediaUpload
                                    onSelect={(media) => setAttributes({
                                        ...
                                        iconImageUrl: media.url || (media.sizes?.full?.url) || '',
                                    })}
                                    allowedTypes={['image']}
                                    value={iconImageId}
                                    render={({ open }) => (
                                                ...
                                                <div style={{ marginBottom: 8 }}>
                                                    ...
                                                    <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                                        ...
                                                        <Button
                                                            variant="secondary"
                                                            isDestructive
                                                            onClick={() => setAttributes({ iconImageId: 0, iconImageUrl: '' })}
                                                        >
                ...
                <PanelBody title={__('Content', 'twork-builder')} initialOpen={false}>
                    <ToggleControl
                        label={__('Show Secondary Line', 'twork-builder')}
                        checked={showSecondary}
                        onChange={(val) => setAttributes({ showSecondary: val })}
                    />
                </PanelBody>

                <PanelBody title={__('CTA', 'twork-builder')} initialOpen={false}>
                    <ToggleControl
                        ...
                        colorSettings={[
                            {
                                ...
                                label: __('Icon Color (override)', 'twork-builder')
                            },
                            {
                                value: iconBackground || undefined,
                                onChange: (val) => setAttributes({ iconBackground: val || '' }),
                                label: __('Icon Background (override)', 'twork-builder')
                            },
                            {
                                value: ctaColor ?? DEFAULT_ATTRS.ctaColor,
                                onChange: (val) => setAttributes({ ctaColor: val }),
                                label: __('CTA Link Color', 'twork-builder')
                            }
                        ]}
                    />
                )}
            ...
            <div {...blockProps}>
                ...
                {showSecondary && (
                    <RichText
                        tagName="p"
                        value={secondaryText}
                        onChange={(val) => setAttributes({ secondaryText: val })}
