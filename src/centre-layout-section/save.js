...
function CentreIconOutput({ iconType, faClass, dashicon, imageUrl, videoUrl, className = '', style = {} }) {
    const wrap = (content) => (
...
export default function save({ attributes }) {
    ...
    const contactWidgetStyle = contactBackgroundImage
        ? { backgroundImage: `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${contactBackgroundImage})`, backgroundSize: 'cover' }
        : undefined;
        ...
        <section {...blockProps}>
            ...
            >
                <div className="jivaka-container layout-grid centre-grid">
                    <aside className="sidebar-wrapper" aria-label="Centre sidebar">
                        <div className="sidebar animate-sidebar">
                            <div className="sidebar-header">{sidebarHeader}</div>
                            <nav className="sidebar-menu" aria-label="Clinical centres">
                                {(navItems || []).map((item, i) => (
                                    <a
                                        key={i}
                                        href={item?.href || '#'}
                        ...
                        {showSidebarContact !== false && (
                            <div className="sidebar-contact animate-sidebar desktop-only" style={contactWidgetStyle}>
                                <CentreIconOutput
                                ...
                                <h3>{contactTitle}</h3>
                                <p>{contactText}</p>
                                <a href={contactPhoneUrl || 'tel:'} className="jivaka-btn btn-primary" style={{ width: '100%' }}>
                                    <CentreIconOutput
                    ...
                    <div className="main-content">
                        <InnerBlocks.Content />
                        {showSidebarContact !== false && (
                            <div className="sidebar-contact animate-sidebar mobile-only" style={contactWidgetStyle}>
                                <CentreIconOutput
                                    iconType={contactIconType || 'fontawesome'}
                                    faClass={contactIcon || 'fas fa-headset'}
                                    dashicon={contactDashicon}
