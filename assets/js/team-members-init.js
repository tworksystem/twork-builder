( function () {
	...
	const initTeamMembers = () => {
		...
		teamSections.forEach( ( section ) => {
			...
			const hoverTranslateY =
				parseFloat( section.dataset.hoverTranslateY ) || -8;
			const hoverScale = parseFloat( section.dataset.hoverScale ) || 1.02;
			const hoverShadow =
				parseFloat( section.dataset.hoverShadow ) || 1.5;
					...
					card.addEventListener( 'mouseenter', function () {
						...
						if ( originalShadow && originalShadow !== 'none' ) {
							...
							if ( shadowParts.length > 0 ) {
								const enhancedShadow = shadowParts
									.map( ( part ) => {
	...
	if ( 'IntersectionObserver' in window ) {
		...
		document
			.querySelectorAll( '.member-image[data-src]' )
			.forEach( ( img ) => {
