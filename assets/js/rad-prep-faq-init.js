...
( function () {
	...
	const SECTION_SELECTOR = '.twork-rad-prep-faq-section';
	const ITEM_SELECTOR = '.rad-faq-item';
	const QUESTION_SELECTOR = '.rad-faq-q';
	const ANSWER_SELECTOR = '.rad-faq-a';
		...
		document
			...
			.forEach( function ( section ) {
				...
				items.forEach( function ( item ) {
					...
					if ( btn.dataset.radPrepFaqBound === '1' ) {
						return;
					}
					...
					btn.addEventListener( 'click', function () {
						...
						items.forEach( function ( other ) {
							const otherBtn =
								other.querySelector( QUESTION_SELECTOR );
							const otherAnswer =
								other.querySelector( ANSWER_SELECTOR );
							if ( otherBtn ) {
				...
				if ( ! anyActive && items[ 0 ] ) {
					const firstBtn =
						items[ 0 ].querySelector( QUESTION_SELECTOR );
					const firstAnswer =
						items[ 0 ].querySelector( ANSWER_SELECTOR );
					if ( firstBtn && firstAnswer ) {
						firstBtn.classList.add( 'active' );
						firstAnswer.style.maxHeight =
