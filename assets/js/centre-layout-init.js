...
(function () {
    ...
    function initFaqAccordions() {
        document.querySelectorAll(SECTION_SELECTOR).forEach(function (section) {
            ...
            items.forEach(function (item) {
                ...
                btn.addEventListener('click', function () {
                    ...
                    items.forEach(function (other) {
                        var otherContent = other.querySelector(FAQ_CONTENT_SELECTOR);
                        if (otherContent) {
            ...
            if (items.length) {
                var firstContent = items[0].querySelector(FAQ_CONTENT_SELECTOR);
                if (firstContent && !firstContent.classList.contains(OPEN_CLASS)) {
    ...
    function initScrollAnimations() {
        ...
        if (!('IntersectionObserver' in window)) {
            sections.forEach(function (section) {
                section.querySelectorAll('.fade-up, .stagger-card, .stagger-doc').forEach(revealEl);
            });
            return;
        }
        ...
        sections.forEach(function (section) {
            ...
            var sectionSections = section.querySelectorAll('.content-section.fade-up, .centre-cta.fade-up');
            sectionSections.forEach(function (el) {
                if (isInViewport(el)) revealEl(el);
            });

            section.querySelectorAll('.content-section.fade-up, .centre-cta.fade-up').forEach(function (el) {
                var observer = new IntersectionObserver(function (entries) {
                    entries.forEach(function (entry) {
                        if (entry.isIntersecting) {
                            requestAnimationFrame(function () { revealEl(entry.target); });
            ...
            if (treatmentGrid && treatmentGrid.querySelectorAll('.stagger-card').length) {
                var cards = treatmentGrid.querySelectorAll('.treatment-card.stagger-card');
                ...
                var gridObserver = new IntersectionObserver(function (entries) {
                    entries.forEach(function (entry) {
                        if (entry.isIntersecting) {
                            var list = entry.target.querySelectorAll('.treatment-card.stagger-card');
                            list.forEach(function (card, i) {
                                card.style.transitionDelay = i * STAGGER_DELAY + 'ms';
                                requestAnimationFrame(function () { revealEl(card); });
                            });
            ...
            if (deptDoctors && deptDoctors.querySelectorAll('.stagger-doc').length) {
                var docCards = deptDoctors.querySelectorAll('.doctor-card-mini.stagger-doc');
                ...
                var docObserver = new IntersectionObserver(function (entries) {
                    entries.forEach(function (entry) {
                        if (entry.isIntersecting) {
                            var list = entry.target.querySelectorAll('.doctor-card-mini.stagger-doc');
                            list.forEach(function (card, i) {
                                card.style.transitionDelay = i * STAGGER_DELAY + 'ms';
                                requestAnimationFrame(function () { revealEl(card); });
                            });
