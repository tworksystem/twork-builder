...
( function () {
	...
	const SECTION_SELECTOR = '.twork-centre-layout-section';
	const FAQ_ITEM_SELECTOR = '.faq-item';
	const FAQ_BTN_SELECTOR = '.faq-btn';
		...
		document
			...
			.forEach( function ( section ) {
				...
				items.forEach( function ( item ) {
					const btn = item.querySelector( FAQ_BTN_SELECTOR );
					const content = item.querySelector( FAQ_CONTENT_SELECTOR );
					if ( ! btn || ! content ) {
						return;
					}
					if ( btn.dataset.centreFaqBound ) {
						return;
					}
	...
	function initScrollAnimations() {
		...
		sections.forEach( function ( section ) {
			...
			const sectionSections = section.querySelectorAll(
			...
			if (
				...
				const cards = treatmentGrid.querySelectorAll(
				...
				var gridObserver = new IntersectionObserver(
					function ( entries ) {
						entries.forEach( function ( entry ) {
							if ( entry.isIntersecting ) {
								const list = entry.target.querySelectorAll(
									'.treatment-card.stagger-card'
								);
			...
			if (
				...
				const docCards = deptDoctors.querySelectorAll(
