( function () {
	...
	const initServicesGrid = () => {
		...
		// Early return if IntersectionObserver is not supported or user prefers reduced motion
		...
		servicesGrids.forEach( ( section, sectionIndex ) => {
			...
			const animationDelay =
				parseInt( section.dataset.animationDelay ) || 100;
			const hoverEnabled = section.dataset.hoverEffect === 'true';
			const hoverTranslateY =
				parseFloat( section.dataset.hoverTranslateY ) || -4;
			...
			if ( animationEnabled && serviceCards.length > 0 ) {
				const animationType =
					section.dataset.animationType || 'fadeInUp';
