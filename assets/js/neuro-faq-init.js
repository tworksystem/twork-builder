        ...
        document.querySelectorAll(SECTION_SELECTOR).forEach(function (section) {
            ...
            items.forEach(function (item) {
                ...
                btn.addEventListener('click', function () {
                    ...
                    items.forEach(function (other) {
                        var otherContent = other.querySelector(CONTENT_SELECTOR);
                        if (otherContent) otherContent.classList.remove(OPEN_CLASS);
                    });
            ...
            if (items.length) {
                ...
                items.forEach(function (item) {
                    ...
                    if (c && c.classList.contains(OPEN_CLASS)) anyOpen = true;
                });
                if (!anyOpen) {
                    var firstContent = items[0].querySelector(CONTENT_SELECTOR);
                    if (firstContent) firstContent.classList.add(OPEN_CLASS);
                }
            }
        });
    }
