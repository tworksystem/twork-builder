	...
	const initHeroNewSection = () => {
		...
		if ( ! prefersReducedMotion ) {
			...
			animatedBlocks.forEach( ( block ) => {
				const hasRevealElements =
					block.querySelectorAll( '.gsap-reveal' ).length > 0;
			...
			// Reduce motion: show hero blocks immediately
		...
		blocks.forEach( ( block ) => {
			...
			const slides = Array.prototype.slice.call(
				slideshow.querySelectorAll( '.hero-bg-slide' )
			);
				...
				if (
					...
					for ( j = 0; j < mutations[ i ].addedNodes.length; j++ ) {
						if ( mutations[ i ].addedNodes[ j ].nodeType === 1 ) {
							if (
								mutations[ i ].addedNodes[ j ].querySelector &&
								mutations[ i ].addedNodes[ j ].querySelector(
									'.hero-bg-slideshow'
								)
							)
