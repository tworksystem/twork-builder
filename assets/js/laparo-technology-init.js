/**
 * Laparoscopy technology — stage image sync from tech-item data attributes.
 */
( function () {
	'use strict';

	const SECTION_SELECTOR = '.twork-laparo-technology-section';
	...
	function setStage( section, items, stages, activeIndex, label ) {
		...
		const hudLabel = section.querySelector( '.laparo-tech-hud-label' );
		const hudIndex = section.querySelector( '.laparo-tech-hud-index' );
		if ( hudLabel && label ) {
	...
	function initTechnologySection( section ) {
		if ( section.dataset.laparoTechnologyBound === '1' ) {
			return;
		}
		section.dataset.laparoTechnologyBound = '1';
		...
		const imageHost = section.querySelector( '.laparo-tech-stage-images' );
		if ( imageHost ) {
	...
	if ( document.readyState === 'loading' ) {
		document.addEventListener( 'DOMContentLoaded', initLaparoTechnology );
	} else {
		initLaparoTechnology();
	}
} )();
