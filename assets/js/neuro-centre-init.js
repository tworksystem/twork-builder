(function () {
    ...
    const initNeuroCentre = () => {
        ...
        if (!('IntersectionObserver' in window)) {
            section.querySelectorAll('.fade-up, .stagger-card, .stagger-doc').forEach((el) => {
                el.style.opacity = '1';
        ...
        section.querySelectorAll('.content-section.fade-up, .neuro-cta.fade-up').forEach((el) => {
            el.style.opacity = '0';
        ...
        section.querySelectorAll('.treatment-card.stagger-card, .doctor-card-mini.stagger-doc').forEach((el) => {
            el.style.opacity = '0';
            el.style.visibility = 'hidden';
            el.style.transform = 'translateY(40px)';
            el.style.transition = 'opacity 0.6s ease-out, transform 0.6s ease-out, visibility 0.6s';
        });
        ...
        if (treatmentGrid && treatmentGrid.querySelectorAll('.stagger-card').length) {
            const observer = new IntersectionObserver((entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        const cards = entry.target.querySelectorAll('.treatment-card.stagger-card');
        ...
        if (deptDoctors && deptDoctors.querySelectorAll('.stagger-doc').length) {
            const observer = new IntersectionObserver((entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        const cards = entry.target.querySelectorAll('.doctor-card-mini.stagger-doc');
                        cards.forEach((card, i) => {
                            card.style.transitionDelay = `${i * STAGGER_DELAY}ms`;
                            requestAnimationFrame(() => {
                                card.style.opacity = '1';
