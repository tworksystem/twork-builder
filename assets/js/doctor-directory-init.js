...
(function () {
    ...
    function filterDoctorsInSection(section, state) {
        ...
        for (i = 0; i < cards.length; i++) {
            ...
            matchesSearch = !state.searchText || name.indexOf(state.searchText) !== -1;
            matchesDept = state.deptValue === 'all' || dept === state.deptValue;
            matchesGender = state.genderValue === 'all' || gender === state.genderValue;
    ...
    function initDoctorDirectorySections() {
        ...
        function getState() {
            return {
                searchText: (els.searchInput && els.searchInput.value) ? els.searchInput.value.toLowerCase().trim() : '',
                deptValue: (els.deptFilter && els.deptFilter.value) ? els.deptFilter.value : 'all',
                genderValue: (els.genderFilter && els.genderFilter.value) ? els.genderFilter.value : 'all'
            };
        }
    ...
    window.TworkDoctorDirectorySection = { init: initDoctorDirectorySections, version: '1.0.1' };
})();
