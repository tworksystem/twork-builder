(function () {
    ...
    const initAmbMapSection = () => {
        if (!('IntersectionObserver' in window)) {
            document.querySelectorAll('.twork-amb-map-section .amb-map-box').forEach((el) => {
                el.style.opacity = '1';
        ...
        const sections = document.querySelectorAll('.twork-amb-map-section[data-animation="true"]');
        if (!sections.length) return;

        sections.forEach((section) => {
            const box = section.querySelector('.amb-map-box.twork-amb-map-box');
            if (!box) return;
