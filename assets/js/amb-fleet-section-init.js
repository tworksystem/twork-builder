(function () {
    ...
    const initAmbFleetSection = () => {
        if (!('IntersectionObserver' in window)) {
            document.querySelectorAll('.twork-amb-fleet-section .amb-fleet-card').forEach((card) => {
        ...
        const sections = document.querySelectorAll('.twork-amb-fleet-section[data-animation="true"]');
        if (!sections.length) return;

        sections.forEach((section) => {
            const cards = section.querySelectorAll('.amb-fleet-card.twork-amb-fleet-card');
